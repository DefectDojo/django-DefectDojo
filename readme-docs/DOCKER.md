...
Alternatively you can run the below command to grab the container ID and change the admin password within one command.
```zsh
docker exec -it $(docker ps --filter name=uwsgi --format {{.ID}}) ./manage.py shell -c 'from django.contrib.auth.models import User; u = User.objects.get(username="admin"); u.set_password("Password123!"); u.save()'
```
