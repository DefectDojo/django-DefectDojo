...
def do_false_positive_history(new_finding, *args, **kwargs):
    ...
    """
    TODO this code retrieves all matching findings + data. in 3 queries.
    just to check if there is a non-zero amount of matching findings.
    if we keep false positive history like this,
    this can be rewritten into 1 query that performs these counts.
    """
...
def deduplicate_legacy(new_finding):
    ...
    for find in total_findings.order_by("id"):
        ...
        if find.endpoints.count() != 0 and new_finding.endpoints.count() != 0:
            ...
            deduplicationLogger.debug(
                "no endpoints on one of the findings and the new finding is either dynamic "
                "or doesn't have a file_path; Deduplication will not occur"
            )
...
def sla_compute_and_notify(*args, **kwargs):
    ...
    try:
        if system_settings.enable_finding_sla:
                ...
                if jira_issue:
                    ...
                    if jira_instance is not None:
                        ...
                        try:
                            ...
                            somehow not working with prefetching though.
                            """
            ...
            logger.info(
                ("SLA run results: Pre-breach: {}, "
                 "at-breach: {}, "
                 "post-breach: {} "
                 "post-breach-no-notify: {}, "
                 "with-jira: {}, "
                 "TOTAL: {}").format(
                    pre_breach_count,
                    at_breach_count,
