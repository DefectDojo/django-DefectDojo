...
def do_dedupe_finding(new_finding, *args, **kwargs):
    ...
    if enabled:
        deduplicationLogger.debug(
            "sync_dedupe for: " + str(new_finding.id) + ":" + str(new_finding.title)
        )
        ...
        if hasattr(settings, "DEDUPLICATION_ALGORITHM_PER_PARSER"):
            ...
            deduplicationLogger.debug(
                "deduplication algorithm: " + deduplicationAlgorithm
            )
        ...
        deduplicationLogger.debug(
            "sync_dedupe: skipping dedupe because it's disabled in system settings get()"
        )
...
def sla_compute_and_notify(*args, **kwargs):
    ...
    def _notify(finding, title):
        create_notification(
            event="sla_breach", title=title, finding=finding, sla_age=sla_age
        )
...
def get_words_for_field(queryset, fieldname):
    ...
    words = [
        word
        for component_name in queryset.order_by()
        .filter(**{"%s__isnull" % fieldname: False})
        .values_list(fieldname, flat=True)
        .distinct()[:max_results]
        for word in (component_name.split() if component_name else [])
        if len(word) > 2
    ]
