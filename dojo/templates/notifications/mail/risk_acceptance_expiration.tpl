		...
		{%block findings%}
			{% for finding in risk_acceptance.accepted_findings.all %}
				{% if not finding.is_mitigated and finding.risk_status == "Risk Accepted" %}
					...
					<a href="{{ finding_url|full_url }}">{{ finding.title }}</a> 
					{% if "PRIORITIZATION_MODEL_SEVERITY"|general_settings_get_value:"True" %}
					{{ finding.severity }} {{ finding|finding_display_status:"email" }}
					{% endif %}
					{% if "PRIORITIZATION_MODEL_PRIORITY"|general_settings_get_value:"True" %}
					{{ finding|render_severity_and_priority }} {{ finding|finding_display_status:"email" }}
					{% endif %}
