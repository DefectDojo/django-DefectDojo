...
# explicitly use our own regex pattern here as django-watson is sensitive
# so we want to control it here independently of models.py etc.
...
def simple_search(request):
    ...
    if request.method == "GET":
        ...
        if form.is_valid():
            ...
            search_vulnerability_ids = (
                "vulnerability_id" in operators or not operators
            )
            ...
            # TODO better get findings in their own query and match on id. that
            # would allow filtering on additional fields such prod_id, etc.
            ...
            if search_finding_id:
                ...
                # we could try to modify request.GET before generating the
                # filter, but for now we'll leave it as is

                title_words = get_words_for_field(Finding, "title")
                component_words = get_words_for_field(
            ...
            # prefetch after watson to avoid inavlid query errors due to watson
            # not understanding prefetching
            ...
            not_tag = (
                operators["not-tag"] if "not-tag" in operators else keywords
            )
            not_tags = (
                operators["not-tags"] if "not-tags" in operators else keywords
            )
            ...
            if search_finding_templates:
                ...
                finding_templates = apply_tag_filters(
                    finding_templates, operators
                )

                if keywords_query:
                    watson_results = watson.filter(
                        finding_templates, keywords_query
                    )
            ...
            if search_vulnerability_ids:
                ...
                if keywords_query:
                    watson_results = watson.filter(
                        vulnerability_ids, keywords_query
                    )
        ...
        add_breadcrumb(
            title=_("Simple Search"), top_level=True, request=request
        )
...
def apply_tag_filters(qs, operators, skip_relations=False):
    ...
    for tag_filter in tag_filters:
        if tag_filter + "s" in operators:
            ...
            qs = qs.filter(
                **{"%stags__name__in" % tag_filters[tag_filter]: value}
            )

    # negative search based on not- prefix (not-tags, not-test-tags,
    # not-engagement-tags, not-product-tags, etc)
...
def apply_endpoint_filter(qs, operators):
    if "endpoint" in operators:
        qs = qs.filter(
            endpoints__host__contains=",".join(operators["endpoint"])
        )
...
def perform_keyword_search_for_operator(
    qs, operators, operator, keywords_query
):
    watson_results = None
    operator_query = ""
