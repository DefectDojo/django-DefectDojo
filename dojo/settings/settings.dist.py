...
env = environ.Env(
    ...
    # You can set extra Jira headers by suppling a dictionary in header: value
    # format (pass as env var like
    # "headr_name=value,another_header=anohter_value")
    DD_ADDITIONAL_HEADERS=(dict, {}),
    # Set fields used by the hashcode generator for deduplication, via en env
    # variable that contains a JSON string
    DD_HASHCODE_FIELDS_PER_SCANNER=(str, ""),
    # Set deduplication algorithms per parser, via en env variable that
...
# 'SLA_NOTIFY_ACTIVE', 'SLA_NOTIFY_ACTIVE_VERIFIED_ONLY' and 'SLA_NOTIFY_WITH_JIRA_ONLY' are moved to system settings,
...
if AUTH_REMOTEUSER_ENABLED:
    ...
    # we need to add middleware just behindAuthenticationMiddleware as
    # described in
    # https://docs.djangoproject.com/en/3.2/howto/auth-remote-user/#configuration
...
HASHCODE_FIELDS_PER_SCANNER = {
    ...
    # possible improvement: in the scanner put the library name into
    # file_path, then dedup on vulnerability_ids + file_path + severity
...
# Default is True (if scanner is not configured here but is configured in
# HASHCODE_FIELDS_PER_SCANNER, it allows null cwe)
...
# Makes it possible to deduplicate on a technical id (same parser) and
# also on some functional fields (cross-parsers deduplication)
...
# '/app/dojo/fixtures/defect_dojo_sample_data.json': 'Settings' object has
# no attribute 'AUDITLOG_DISABLE_ON_RAW_SAVE'
