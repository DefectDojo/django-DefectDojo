...
class ReviewFindingForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if finding is not None and settings.FEATURE_AUTHORIZATION_V2:
            self.fields['reviewers'].queryset = get_authorized_users_for_product_and_product_type(None,
...
class UserContactInfoForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not current_user.is_superuser:
            ...
            if settings.USER_PROFILE_READ_ONLY:
                for field in self.fields:
                    self.fields[field].disabled = True
...
class AssignUserForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if assignee is None:
            self.fields['assignee'] = forms.ModelChoiceField(queryset=Dojo_User.objects.all(),
