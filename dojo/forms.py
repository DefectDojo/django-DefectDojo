...
class Product_TypeForm(forms.ModelForm):
    ...
    authorized_users = forms.ModelMultipleChoiceField(
        queryset=None, required=False, label="Authorized Users"
    )

    def __init__(self, *args, **kwargs):
        non_staff = (
            Dojo_User.objects.exclude(is_staff=True)
...
class ProductForm(forms.ModelForm):
    ...
    app_analysis = forms.ModelMultipleChoiceField(
        label="Technologies",
...
class ImportScanForm(forms.Form):
    ...
    verified = forms.BooleanField(
        help_text="Select if these findings have been verified.", required=False
    )
    ...
    version = forms.CharField(
        max_length=100,
        required=False,
        help_text="Version that will be set on the Test object that will be created.",
    )
    ...
    file = forms.FileField(
        widget=forms.widgets.FileInput(
            attrs={
                "accept": ".xml, .csv, .nessus, .json, .html, .js, .zip, .xlsx, .txt, .sarif"
            }
        ),
...
class ReImportScanForm(forms.Form):
    ...
    close_old_findings = forms.BooleanField(
        help_text="Select if old findings get mitigated when importing.",
        required=False,
        initial=True,
    )
    version = forms.CharField(
...
ManageFileFormSet = modelformset_factory(
    FileUpload, extra=3, max_num=10, fields=["title", "file"], can_delete=True
)
...
class VaForm(forms.ModelForm):
    ...
    options = (("Immediately", "Immediately"), ("6AM", "6AM"), ("10PM", "10PM"))
...
class EngForm(forms.ModelForm):
    ...
    product = forms.ModelChoiceField(
        label="Product", queryset=Product.objects.all().order_by("name"), required=True
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if product:
            ...
            staff_users = [
                ...
                if user_is_authorized(user, "staff", product)
            ]
        ...
        if (
            ...
            self.fields["product"].queryset = Product.objects.all().filter(
    ...
    def __init__(self, *args, **kwargs):
        ...
        if obj:
            staff_users = [
                user.id
                for user in User.objects.all()
                if user_is_authorized(user, "staff", obj)
            ]
        else:
...
FindingImageFormSet = modelformset_factory(
    FindingImage, extra=3, max_num=10, exclude=[""], can_delete=True
)
...
class Sonarqube_ProductForm(forms.ModelForm):
    ...
    sonarqube_tool_config = forms.ModelChoiceField(
        ...
        queryset=Tool_Configuration.objects.filter(
            tool_type__name="SonarQube"
        ).order_by("name"),
        required=False,
    )
...
class JIRAProjectForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        raise ValidationError(
            "JIRA Project needs a JIRA Instance and JIRA Project Key, or leave both empty"
        )
...
class JIRAFindingForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        self.fields["jira_issue"].widget = forms.TextInput(
            attrs={
                "placeholder": "Leave empty and check push to jira to create a new JIRA issue"
            }
        )
...
class DeleteGeneralQuestionnaireForm(forms.ModelForm):
    ...
    class Meta:
        model = General_Survey
        exclude = ("num_responses", "generated", "expiration", "survey")
...
class CreateQuestionForm(forms.Form):
    ...
    order = forms.IntegerField(
        min_value=1, widget=forms.TextInput(attrs={"data-type": "both"})
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if assignee is None:
            self.fields["assignee"] = forms.ModelChoiceField(
                queryset=User.objects.all(), empty_label="Not Assigned", required=False
            )
        else:
