...
class MultipleSelectWithPopPlusMinus(forms.SelectMultiple):
    def render(self, name, *args, **kwargs):
        ...
        popup_plus = '<div class="input-group dojo-input-group">' + html + '<span class="input-group-btn"><a href="/' \
                     + name + '/add" class="btn btn-primary" class="add-another" id="add_id_' + name + '" onclick="return showAddAnotherPopup(this);"><span class="icon-plusminus"></span></a></span></div>'
...
class ProductForm(forms.ModelForm):
    ...
    class Meta:
        model = Product
        if settings.FEATURE_AUTHORIZATION_V2:
            fields = ['name', 'description', 'tags', 'product_manager', 'technical_contact', 'team_manager',
                      'prod_type', 'regulations', 'app_analysis',
            ...
            fields = ['name', 'description', 'tags', 'product_manager', 'technical_contact', 'team_manager',
                      'prod_type', 'regulations', 'app_analysis',
                      'authorized_users', 'business_criticality', 'platform', 'lifecycle', 'origin', 'user_records',
                      'revenue', 'external_audience',
                      'internet_accessible', 'enable_simple_risk_acceptance', 'enable_full_risk_acceptance']
...
class ImportScanForm(forms.Form):
    ...
    close_old_findings = forms.BooleanField(
        help_text="Select if old findings no longer present in the report get mitigated when importing."
                  "This affects the whole engagement/product depending on your deduplication scope.",
        required=False, initial=False)

    if settings.FEATURE_FINDING_GROUPS:
        group_by = forms.ChoiceField(required=False, choices=Finding_Group.GROUP_BY_OPTIONS,
                                     help_text='Choose an option to automatically group new findings by the chosen '
                                               'option.')
...
class ReImportScanForm(forms.Form):
    ...
    version = forms.CharField(max_length=100, required=False,
                              help_text="Version that will be set on existing Test object. Leave empty to leave "
...
class MergeFindings(forms.ModelForm):
    ...
    append_description = forms.BooleanField(label="Append Description", initial=True, required=False,
                                            help_text="Description in all findings will be appended into the merged "
                                                      "finding.")
    ...
    dynamic_raw = forms.BooleanField(label="Dynamic Scanner Raw Requests", initial=True, required=False,
                                     help_text="Dynamic scanner raw requests in all findings will be merged into the "
                                               "merged finding.")
    ...
    mark_tag_finding = forms.BooleanField(label="Tag Merged Finding", initial=True, required=False,
                                          help_text="Creates a tag titled 'merged' for the finding that will be "
                                                    "merged. If the 'Finding Action' is set to 'inactive' the "
                                                    "inactive findings will be tagged with 'merged-inactive'.")

    append_reference = forms.BooleanField(label="Append Reference", initial=True, required=False,
                                          help_text="Reference in all findings will be appended into the merged "
                                                    "finding.")
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['finding_to_merge_into'] = forms.ModelChoiceField(
            queryset=findings, initial=0, required="False", label="Finding to Merge Into",
        ...
        self.fields.field_order = ['finding_to_merge_into', 'findings_to_merge', 'append_description',
                                   'add_endpoints', 'append_reference']
...
class EditRiskAcceptanceForm(forms.ModelForm):
    ...
    recommendation = forms.ChoiceField(choices=Risk_Acceptance.TREATMENT_CHOICES,
                                       initial=Risk_Acceptance.TREATMENT_ACCEPT, widget=forms.RadioSelect,
                                       label="Security Recommendation")
    decision = forms.ChoiceField(choices=Risk_Acceptance.TREATMENT_CHOICES, initial=Risk_Acceptance.TREATMENT_ACCEPT,
                                 widget=forms.RadioSelect)

    path = forms.FileField(label="Proof", required=False,
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields[
            'path'].help_text = 'Existing proof uploaded: %s' % self.instance.filename() if self.instance.filename() \
            else 'None'
...
class RiskAcceptanceForm(EditRiskAcceptanceForm):
    # path = forms.FileField(label="Proof", required=False, widget=forms.widgets.FileInput(attrs={"accept": ".jpg,
    # .png,.pdf"}))
    # expiration_date = forms.DateTimeField(required=False, widget=forms.TextInput(attrs={'class': 'datepicker'}))
    ...
    def __init__(self, *args, **kwargs):
        ...
        # self.fields['path'].help_text = 'Existing proof uploaded: %s' % self.instance.filename() if
        # self.instance.filename() else 'None'
...
class ReplaceRiskAcceptanceProofForm(forms.ModelForm):
    path = forms.FileField(label="Proof", required=True,
                           widget=forms.widgets.FileInput(attrs={"accept": ".jpg,.png,.pdf"}))
    ...
    def __init__(self, *args, **kwargs):
        ...
        if product:
            self.fields['preset'] = forms.ModelChoiceField(
                help_text="Settings and notes for performing this engagement.", required=False,
                queryset=Engagement_Presets.objects.filter(product=product))
            if not settings.FEATURE_AUTHORIZATION_V2:
                authorized_for_lead = [user.id for user in User.objects.all() if
                                       user_is_authorized(user, 'staff', product)]
                self.fields['lead'].queryset = User.objects.filter(id__in=authorized_for_lead)
            else:
                self.fields['lead'].queryset = get_authorized_users_for_product_and_product_type(None, product,
...
class AddFindingForm(forms.ModelForm):
    ...
    cvssv3 = forms.CharField(max_length=117, required=False, widget=forms.TextInput(
        attrs={'class': 'cvsscalculator', 'data-toggle': 'dropdown', 'aria-haspopup': 'true',
               'aria-expanded': 'false'}))
...
class PromoteFindingForm(forms.ModelForm):
    ...
    class Meta:
        ...
        exclude = ('reporter', 'url', 'numerical_severity', 'endpoint', 'active', 'false_p', 'verified', 'is_template',
                   ...
                   'is_mitigated', 'jira_creation', 'jira_change')
...
class FindingForm(forms.ModelForm):
    title = forms.CharField(max_length=1000)
    group = forms.ModelChoiceField(required=False, queryset=Finding_Group.objects.none(),
                                   help_text='The Finding Group to which this finding belongs, leave empty to remove '
                                             'the finding from the group. Groups can only be created via Bulk Edit '
                                             'for now.')
    date = forms.DateField(required=True,
                           widget=forms.TextInput(attrs={'class': 'datepicker', 'autocomplete': 'off'}))
    cwe = forms.IntegerField(required=False)
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.instance.test.engagement.product.enable_simple_risk_acceptance):
            ...
            if self.instance.risk_accepted:
                self.fields[
                    'risk_accepted'].help_text = "Uncheck to unaccept the risk. Use full risk acceptance from the " \
                                                 "dropdown menu if you need advanced settings such as an expiry date."
            elif self.instance.test.engagement.product.enable_simple_risk_acceptance:
                self.fields[
                    'risk_accepted'].help_text = "Check to accept the risk. Use full risk acceptance from the " \
                                                 "dropdown menu if you need advanced settings such as an expiry date."
...
class ApplyFindingTemplateForm(forms.Form):
    ...
    cvssv3 = forms.CharField(label="CVSSv3", max_length=117, required=False, widget=forms.TextInput(
        attrs={'class': 'btn btn-secondary dropdown-toggle', 'data-toggle': 'dropdown', 'aria-haspopup': 'true',
               'aria-expanded': 'false'}))

    severity = forms.ChoiceField(required=False, choices=SEVERITY_CHOICES,
                                 error_messages={'required': 'Select valid choice: In Progress, On Hold, Completed',
                                                 'invalid_choice': 'Select valid choice: Critical,High,Medium,Low'})
    ...
    tags = TagField(required=False,
                    help_text="Add tags that help describe this finding template. Choose from the list or add new "
    ...
    class Meta:
        fields = ['title', 'cwe', 'cve', 'cvssv3', 'severity', 'description', 'mitigation', 'impact', 'references',
                  'tags']
        order = ('title', 'cwe', 'cve', 'cvssv3', 'severity', 'description', 'impact', 'is_mitigated')


class FindingTemplateForm(forms.ModelForm):
    apply_to_findings = forms.BooleanField(required=False,
                                           help_text="Apply template to all findings that match this CWE. (Update "
                                                     "will overwrite mitigation, impact and references for any "
    ...
    field_order = ['title', 'cwe', 'cve', 'severity', 'cvssv3', 'description', 'mitigation', 'impact', 'references',
                   'tags', 'template_match', 'template_match_cwe', 'template_match_title', 'apply_to_findings']
...
class ExpressGITHUBForm(forms.ModelForm):
    ...
    issue_key = forms.CharField(required=True,
...
class JIRAForm(forms.ModelForm):
    issue_template_dir = forms.ChoiceField(required=False,
                                           ...
                                           help_text='Choose the folder containing the Django templates used to '
                                                     'render the JIRA issue description. These are stored in '
                                                     'dojo/templates/issue-trackers. Leave empty to use the default '
                                                     'jira_full templates.')
...
class Benchmark_Product_SummaryForm(forms.ModelForm):
    class Meta:
        ...
        exclude = ['product', 'current_level', 'benchmark_type', 'asvs_level_1_benchmark', 'asvs_level_1_score',
                   'asvs_level_2_benchmark', 'asvs_level_2_score', 'asvs_level_3_benchmark', 'asvs_level_3_score']
...
class ProductNotificationsForm(forms.ModelForm):
    ...
    class Meta:
        model = Notifications
        fields = ['engagement_added', 'close_engagement', 'test_added', 'scan_added', 'sla_breach',
...
class JIRAProjectForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ['inherit_from_product', 'jira_instance', 'project_key', 'issue_template_dir', 'component',
                  'push_all_issues', 'enable_engagement_epic_mapping', 'push_notes', 'product_jira_sla_notification',
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.target == 'engagement':
            product_name = self.product.name if self.product else self.engagement.product.name if \
                self.engagement.product else ''

            self.fields['project_key'].widget = forms.TextInput(
                attrs={'placeholder': 'JIRA settings inherited from product ''%s''' % product_name})
            self.fields[
                'project_key'].help_text = 'JIRA settings are inherited from product ''%s'', unless configured ' \
                                           'differently here.' % product_name
            self.fields[
                'jira_instance'].help_text = 'JIRA settings are inherited from product ''%s'' , unless configured ' \
                                             'differently here.' % product_name
            ...
            if self.instance.id:
                ...
                self.fields[
                    'inherit_from_product'].help_text = 'Once an engagement has a JIRA Project stored, you cannot ' \
                                                        'switch back to inheritance to avoid breaking existing JIRA ' \
                                                        'issues'
                ...
                if jira_project_product and not (self.prefix + '-jira_instance') in self.data:
                    ...
                    self.initial[
                        'jira_instance'] = jira_project_product.jira_instance.id if \
                        jira_project_product.jira_instance else None
        ...
        if not self.cleaned_data.get('inherit_from_product', False):
            ...
            if self.target == 'engagement':
                raise ValidationError(
                    'JIRA Project needs a JIRA Instance and JIRA Project Key, or choose to inherit settings from '
                    'product')
            else:
                raise ValidationError(
                    'JIRA Project needs a JIRA Instance and JIRA Project Key, leave empty to have no JIRA integration '
                    'setup')
...
class GITHUBFindingForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        self.fields[
            'push_to_github'].help_text = "Checking this will overwrite content of your Github issue, or create one."
...
class JIRAFindingForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        if settings.FEATURE_FINDING_GROUPS:
            self.fields[
                'push_to_jira'].help_text = "Checking this will overwrite content of your JIRA issue, or create one. " \
                                            "If this finding is part of a Finding Group, the group will pushed " \
    ...
    def clean(self):
        ...
        if self.cleaned_data.get('push_to_jira', None) and finding.has_jira_group_issue:
            can_be_pushed_to_jira, error_message, error_code = jira_helper.can_be_pushed_to_jira(
                self.instance.finding_group, self.finding_form)
            ...
            can_be_pushed_to_jira, error_message, error_code = jira_helper.can_be_pushed_to_jira(self.instance,
                                                                                                 self.finding_form)
            if not can_be_pushed_to_jira:
                self.add_error('push_to_jira', ValidationError(error_message, code=error_code))
                # for field in error_fields:
        ...
        if jira_issue_key_new and not finding.has_jira_group_issue:
            ...
            if jira_issue_need_to_exist:
                ...
                jira_issues = JIRA_Issue.objects.filter(jira_id=jira_issue_new.id, jira_key=jira_issue_key_new).exclude(
...
class JIRAEngagementForm(forms.Form):
    ...
    push_to_jira = forms.BooleanField(required=False, label="Create EPIC",
...
class EditQuestionnaireQuestionsForm(forms.ModelForm):
    questions = forms.ModelMultipleChoiceField(
        ...
        required=True,
        help_text="Select questions to include on this questionnaire.  Field can be used to search available "
                  "questions.",
