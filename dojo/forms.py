...
class JIRAProjectForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.target == 'engagement':
            ...
            self.fields['project_key'].widget = forms.TextInput(attrs={'placeholder': 'JIRA settings inherited from product 'f'{product_name}'''})
            self.fields['project_key'].help_text = 'JIRA settings are inherited from product 'f'{product_name}'', unless configured differently here.'
            self.fields['jira_instance'].help_text = 'JIRA settings are inherited from product 'f'{product_name}'' , unless configured differently here.'
