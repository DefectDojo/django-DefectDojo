...
class Command(BaseCommand):
    help = (
        "Command to move data from some tables to other tables as part of "
        "https://github.com/DefectDojo/django-DefectDojo/pull/3200"
        + "Should normally be handled by the migration in that PR, but if that causes errors, "
        "this command can help to get the data migrated anyway."
    )
    ...
    def move_jira_creation_changed(self):
        ...
        for jira_issue in JIRA_Issue.objects.all().select_related("finding"):
            ...
            if jira_issue.finding:
                ...
                logger.debug(
                    "no finding: skipping jira_issue: %s", jira_issue.jira_key
                )
    ...
    def populate_jira_project(self):
        ...
        for jira_issue in (
            ...
            if not jira_issue.jira_project and jira_issue.finding:
                logger.info(
                    "populating jira_issue from finding: %s",
                    jira_issue.jira_key,
                )
                ...
                logger.debug(
                    "populating jira_issue from engagement: %s",
                    jira_issue.jira_key,
                )
                jira_project = jira_helper.get_jira_project(jira_issue.finding)
    ...
    def populate_jira_instance_name_if_empty(self):
        logger.info(
            "populating JIRA_Instance.configuration_name with url if empty"
        )
    ...
    def handle(self, *args, **options):
        ...
        logger.info(
            (
                "now this script is completed, you can run the migration 0063_jira_refactor_populate as normal. "
                "it will skip over the data because it has already been populated"
            )
        )
        logger.info(
            (
                "if it still fails, comment out all the runpython parts, "
                "but leave the operations on the database fields in place"
            )
        )
