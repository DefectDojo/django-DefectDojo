...
def generate_hash_code(f):
    ...
    if f.hash_code != old_hash_code:
        logger.debug(
            "%d: hash_code changed from %s to %s",
            f.id,
            old_hash_code,
            f.hash_code,
        )
...
class Command(BaseCommand):
    """
    Updates hash codes and/or runs deduplication for findings.
    Hashcode calculation always runs in the foreground,
    dedupe by default runs in the background.
    Usage: manage.py dedupe [--parser "Parser1 Scan" --parser "Parser2 Scan"...]
            ...
            if get_system_setting("enable_deduplication"):
                ...
                logger.info(
                    ...
                    (
                        "foreground"
                        if dedupe_sync
                        else "tasks submitted to celery"
                    ),
                )
            else:
                logger.debug(
