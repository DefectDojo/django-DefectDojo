...
def jira_status_reconciliation(*args, **kwargs):
    ...
    messages = [
        (
            "jira_key;finding_url;resolution_or_status;find.jira_issue.jira_change;issue_from_jira."
            "fields.updated;find.last_status_update;issue_from_jira.fields.updated;find.last_reviewed;"
            "issue_from_jira.fields.updated;flag1;flag2;flag3;action;change_made"
        )
    ]
    for find in findings:
        ...
        issue_from_jira.fields.updated = parse_datetime(
        ...
        if mode == "reconcile" and not find.last_status_update:
            ...
            if mode in ("push_status_to_jira", "import_status_from_jira"):
                ...
                logger.debug(
                    "%s,%s,%s,%s", resolution_name, flag1, flag2, flag3
                )
            ...
            if action == "import_status_from_jira":
                message_action = (
                    "deactivating" if find.active else "reactivating"
                )
                ...
                if status_changed:
                    ...
                    message = (
                        "%s; %s/finding/%d;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;"
                        "no changes made while pushing status to jira;%s"
                    ) % (
                        find.jira_issue.jira_key,
                        settings.SITE_URL,
                        find.id,
...
class Command(BaseCommand):
    ...
    modes:
    - reconcile: reconcile any differences in status between Defect Dojo and JIRA, will look at the latest status update
    in Defect Dojo and the 'updated' field in the JIRA Issue.
    ...
    mode_help = (
        "- reconcile: (default)reconcile any differences in status between Defect Dojo and JIRA,"
        "will look at the latest status"
        "change timestamp in both systems to determine which one is the correct status"
        "- push_status_to_jira: update JIRA status for all JIRA issues connected to a Defect Dojo finding \
            (will not push summary/description, only status)"
