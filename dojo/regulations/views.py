...
def new_regulation(request):
    if request.method == "POST":
        ...
        add_breadcrumb(
            title="New regulation", top_level=False, request=request
        )
...
def edit_regulations(request, ttid):
    ...
    if request.method == "POST" and request.POST.get("delete"):
        user_has_configuration_permission_or_403(
            request.user, "dojo.delete_regulation"
        )
...
def regulations(request):
    ...
    add_breadcrumb(
        title="Regulations", top_level=not len(request.GET), request=request
    )
    return render(
        request,
