...
# for examples of incoming json, see the unit tests for the
# webhook: https://github.com/DefectDojo/django-DefectDojo/blob/master/unittests/test_jira_webhook.py
...
def webhook(request, secret=None):
    ...
    # if webhook secret is disabled in system_settings, we ignore the incoming
    # secret, even if it doesn't match
        ...
        try:
            ...
            if parsed.get("webhookEvent") == "jira:issue_updated":
                ...
                if assignee is not None:
                    # First look for the 'name' field. If not present, try
                    # 'displayName'. Else put None
                ...
                resolution = (
                    resolution if resolution and resolution != "None" else None
                )
            ...
            if parsed.get("webhookEvent") == "comment_created":
                ...
                # example: body['comment']['self'] =
                # "http://www.testjira.com/jira_under_a_path/rest/api/2/issue/666/comment/456843"
                ...
                for jira_userid in jira_usernames:
                    # logger.debug('incoming username: %s jira config username: %s',
                    # commentor.lower(), jira_userid.lower())
                    if jira_userid.lower() == commentor.lower():
                        logger.debug(
                            (
                                "skipping incoming JIRA comment as the user id of the comment in JIRA (%s) "
                                "matches the JIRA username in DefectDojo (%s)"
                            ),
                            commentor.lower(),
                            jira_userid.lower(),
                        )
    ...
    if request.method == "POST":
        ...
        if jform.is_valid():
            ...
            try:
                ...
                return render(
                    request, "dojo/express_new_jira.html", {"jform": jform}
                )
            ...
            try:
                ...
                messages.add_message(
                    ...
                    messages.ERROR,
                    (
                        "Unable to find Open/Close ID's (invalid issue key specified?). "
                        "They will need to be found manually"
                    ),
                    extra_tags="alert-danger",
                )
        ...
        add_breadcrumb(
            title="New Jira Configuration (Express)",
            top_level=False,
            request=request,
        )
...
def new_jira(request):
    if request.method == "POST":
        ...
        if jform.is_valid():
            ...
            jira_helper.get_jira_connection_raw(
                jira_server, jira_username, jira_password
            )
        ...
        jform = JIRAForm()
        add_breadcrumb(
            title="New Jira Configuration", top_level=False, request=request
        )
    return render(request, "dojo/new_jira.html", {"jform": jform})
...
def jira(request):
    ...
    add_breadcrumb(
        title="JIRA List", top_level=not len(request.GET), request=request
    )
    return render(
        request, "dojo/jira.html", {"jira_instances": jira_instances}
    )
