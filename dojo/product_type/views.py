...
def view_product_type(request, ptid):
    ...
    groups = get_authorized_groups_for_product_type(
        pt, Permissions.Product_Type_View
    )
    products = get_authorized_products(Permissions.Product_View).filter(
        prod_type=pt
    )
...
def delete_product_type(request, ptid):
    ...
    add_breadcrumb(
        title=_("Delete Product Type"), top_level=False, request=request
    )
    return render(
        request,
...
@user_is_authorized(
    Product_Type, Permissions.Product_Type_Manage_Members, "ptid"
)
def add_product_type_member(request, ptid):
    ...
    if request.method == "POST":
        ...
        if memberform.is_valid():
            if memberform.cleaned_data[
                "role"
            ].is_owner and not user_has_permission(
...
def edit_product_type_member(request, memberid):
    ...
    if request.method == "POST":
        memberform = Edit_Product_Type_MemberForm(
        ...
        if memberform.is_valid():
            ...
            if member.role.is_owner and not user_has_permission(
                ...
                if is_title_in_breadcrumbs("View User"):
                    ...
                    return HttpResponseRedirect(
                        reverse(
                            "view_product_type", args=(member.product_type.id,)
                        )
                    )
    add_breadcrumb(title=page_name, top_level=False, request=request)
    return render(
...
def delete_product_type_member(request, memberid):
    ...
    if request.method == "POST":
        memberform = Delete_Product_Type_MemberForm(
            request.POST, instance=member
        )
...
@user_is_authorized(
    Product_Type_Group, Permissions.Product_Type_Group_Edit, "groupid"
)
        ...
        if groupform.is_valid():
            if group.role.is_owner and not user_has_permission(
                ...
                if is_title_in_breadcrumbs("View Group"):
                    ...
                    return HttpResponseRedirect(
                        reverse(
                            "view_product_type", args=(group.product_type.id,)
                        )
                    )
