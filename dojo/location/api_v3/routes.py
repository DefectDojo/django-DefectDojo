...
def build_locations_router(
    *,
    ...
    def list_locations(request: HttpRequest):
        ...
        # ?fields= may opt up into the detail field set (§4.7 Part A); the URL-subtype detail fields
        # are read through the `url` reverse-O2O (not own-model columns) so nothing is deferrable, but
        # the join is added only when such a field is requested (Part B / DETAIL_SELECT_RELATED).
