    ...
    if not system_settings.allow_anonymous_survey_repsonse:
        ...
        if not auth:
            messages.add_message(
                ...
                messages.ERROR,
                (
                    "You must be authorized to answer questionnaire. Otherwise, "
                    "enable anonymous response in system settings."
                ),
                extra_tags="alert-danger",
            )
...
def assign_questionnaire(request, eid, sid):
    ...
    add_breadcrumb(
        title="Assign Questionnaire", top_level=False, request=request
    )
    return render(
        request,
...
def edit_questionnaire(request, sid):
    ...
    if len(answered) > 0:
        messages.add_message(
            ...
            messages.ERROR,
            (
                "This questionnaire already has answered instances. "
                "If you change it, the responses may no longer be valid."
            ),
    ...
    add_breadcrumb(
        title="Edit Questionnaire", top_level=False, request=request
    )
    return render(
        request,
...
def delete_questionnaire(request, sid):
    ...
    add_breadcrumb(
        title="Delete Questionnaire", top_level=False, request=request
    )
    return render(
        request,
        "defectDojo-engagement-survey/delete_questionnaire.html",
...
def create_questionnaire(request):
    ...
    add_breadcrumb(
        title="Create Questionnaire", top_level=False, request=request
    )
    return render(
        request,
        "defectDojo-engagement-survey/create_questionnaire.html",
...
def edit_questionnaire_questions(request, sid):
    ...
    add_breadcrumb(
        title="Update Questionnaire Questions",
        top_level=False,
        request=request,
    )
    return render(
...
def add_empty_questionnaire(request):
    ...
    form.fields["survey"].queryset = surveys
    add_breadcrumb(
        title="Add Empty Questionnaire", top_level=False, request=request
    )
    return render(
        request,
        "defectDojo-engagement-survey/add_survey.html",
    ...
    if not settings.allow_anonymous_survey_repsonse:
        if not request.user.is_authenticated:
            messages.add_message(
                ...
                messages.ERROR,
                (
                    "You must be logged in to answer questionnaire. Otherwise, "
                    "enable anonymous response in system settings."
                ),
                extra_tags="alert-danger",
            )
