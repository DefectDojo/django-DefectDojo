...
def dashboard(request: HttpRequest) -> HttpResponse:
    ...
    date_range = [
        ...
        today,
    ]  # 7 days (6 days plus today)
    finding_count = findings.filter(created__date__range=date_range).count()
    mitigated_count = findings.filter(
        mitigated__date__range=date_range
    ).count()
    ...
    punchcard, ticks = get_punchcard_data(
        findings, today - relativedelta(weeks=26), 26
    )

    if user_has_configuration_permission(
        request.user, "dojo.view_engagement_survey"
    ):
...
def support(request: HttpRequest) -> HttpResponse:
    add_breadcrumb(
        title="Support", top_level=not len(request.GET), request=request
    )
...
def get_severities_all(findings) -> Dict[str, int]:
    ...
    return defaultdict(
        lambda: 0, {s["severity"]: s["count"] for s in severities_all}
    )
