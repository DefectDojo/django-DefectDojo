    ...
    if risk_acceptance.reactivate_expired:
        for finding in risk_acceptance.accepted_findings.all():
            if not finding.active:
                ...
                # findings remain in this risk acceptance for reporting /
                # metrics purposes
            else:
                logger.debug(
                    "%i:%s already active, no changes made.",
                    finding.id,
                    finding,
                )
...
def reinstate(risk_acceptance, old_expiration_date):
    if risk_acceptance.expiration_date_handled:
        ...
        for finding in risk_acceptance.accepted_findings.all():
            if finding.active:
                logger.debug(
                    "%i:%s: accepting a.k.a. deactivating finding",
                    finding.id,
                    finding,
                )
...
def remove_finding_from_risk_acceptance(risk_acceptance, finding):
    logger.debug(
        "removing finding %i from risk acceptance %i",
        finding.id,
        risk_acceptance.id,
    )
...
def expiration_warning_message_creator(risk_acceptance, heads_up_days=0):
    return (
        "Risk acceptance [(%s)|%s] with %i findings will expire in %i days"
        % (
            escape_for_jira(risk_acceptance.name),
            get_full_url(
                reverse(
...
def unaccepted_message_creator(risk_acceptance, heads_up_days=0):
    if risk_acceptance:
        return (
            "finding was unaccepted/deleted from risk acceptance [(%s)|%s]"
            % (
                escape_for_jira(risk_acceptance.name),
                get_full_url(
                    reverse(
...
def post_jira_comments(
    risk_acceptance, findings, message_factory, heads_up_days=0
):
...
def risk_unaccept(finding, perform_save=True):
    ...
    if finding.risk_accepted:
        ...
        if (
            not finding.mitigated
            and not finding.false_p
            and not finding.out_of_scope
        ):
            finding.active = True
        finding.risk_accepted = False
        if perform_save:
            logger.debug(
                "saving unaccepted finding %i:%s", finding.id, finding
            )
            finding.save(dedupe_option=False)
