...
class AcceptedRiskSerializer(serializers.Serializer):
    vulnerability_id = serializers.CharField(
        ...
        help_text="An id of a vulnerability in a security advisory associated with this finding.\
            Can be a Common Vulnerabilities and Exposure (CVE) or from other sources.",
    )
    ...
    accepted_by = serializers.CharField(
        max_length=200,
        help_text="Name or email of person who accepts the risk",
    )
...
class AcceptedRisksMixin(ABC):
    ...
    @swagger_auto_schema(
        ...
        responses={
            status.HTTP_201_CREATED: RiskAcceptanceSerializer(many=True)
        },
    )
    @extend_schema(
...
class AcceptedFindingsMixin(ABC):
    ...
    def accept_risks(self, request):
        ...
        owner = request.user
        accepted_result = []
        for engagement in get_authorized_engagements(
...
def _accept_risks(
    ...
    for risk in accepted_risks:
        ...
        if findings.exists():
            # TODO we could use risk.vulnerability_id to name the risk_acceptance, but would need \
            # to check for existing risk_acceptances in that case
