...
def update_import_history(
    ...
    if endpoints_to_add:
        import_settings["endpoints"] = [
            str(endpoint) for endpoint in endpoints_to_add
        ]
    ...
    for finding in closed_findings:
        logger.debug(
            "preparing Test_Import_Finding_Action for closed finding: %i",
            finding.id,
        )
    ...
    for finding in new_findings:
        logger.debug(
            "preparing Test_Import_Finding_Action for created finding: %i",
            finding.id,
        )
        test_import_finding_action_list.append(
            Test_Import_Finding_Action(
    ...
    for finding in untouched_findings:
        logger.debug(
            "preparing Test_Import_Finding_Action for untouched finding: %i",
            finding.id,
        )
        test_import_finding_action_list.append(
            Test_Import_Finding_Action(
...
def construct_imported_message(
    ...
    if finding_count:
        ...
        if reactivated_finding_count:
            message = message + " reactivated %d findings" % (
                reactivated_finding_count
            )
        if untouched_finding_count:
            message = message + " did not touch %d findings" % (
        ...
        message = (
            "No findings were added/updated/closed/reactivated as the findings "
            "in Defect Dojo are identical to those in the uploaded report."
        )
...
def chunk_list(list):
    ...
    chunk_list = [
        list[i : i + chunk_size] for i in range(0, len(list), chunk_size)
    ]
    logger.debug(
...
def chunk_endpoints_and_disperse(finding, test, endpoints, **kwargs):
    if settings.ASYNC_FINDING_IMPORT:
        ...
        if len(chunked_list) < 2:
            add_endpoints_to_unsaved_finding(
                finding, test, endpoints, sync=True
            )
            return []
        # First kick off all the workers
        for endpoints_list in chunked_list:
            add_endpoints_to_unsaved_finding(
                finding, test, endpoints_list, sync=False
            )
    else:
