...
class BaseImporter(ImporterOptions):
    ...
    def finding_cwe_numbers(self, finding: Finding) -> list[int]:
        """Primary Finding.cwe plus any parser-supplied unsaved_cwes, positive and deduplicated."""
        cwe_numbers = []
    ...
    def store_cwes(self, finding: Finding) -> None:
        ...
        self.pending_cwes.extend([
            Finding_CWE(finding=finding, cwe=cwe) for cwe in self.finding_cwe_numbers(finding)
        ])
    ...
    def reconcile_cwes(self, finding: Finding) -> None:
        ...
        new_cwes = set(self.finding_cwe_numbers(finding))
