...
def update_endpoint_status(existing_finding, new_finding, user):
    ...
    # using `.all()` will mark as mitigated also `endpoint_status` with flags `false_positive`, `out_of_scope` and `risk_accepted`. This is known issue. This is not a bug. This is future.
