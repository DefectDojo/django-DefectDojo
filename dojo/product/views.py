...
def product(request):
    ...
    # perform all stuff for filtering and pagination first, \
    # before annotation/prefetching
    # otherwise the paginator will perform all the annotations/prefetching \
    # already only to count the total number of records
    ...
    if isinstance(
        ...
        if get_system_setting("enable_github"):
            prefetched_prods = prefetched_prods.prefetch_related(
                Prefetch(
                    "github_pkey_set",
                    queryset=GITHUB_PKey.objects.all().select_related(
                        "git_conf"
                    ),
...
def view_product(request, pid):
    ...
    product_members = get_authorized_members_for_product(
        prod, Permissions.Product_View
    )
    product_type_members = get_authorized_members_for_product_type(
        prod.prod_type, Permissions.Product_Type_View
    )
    product_groups = get_authorized_groups_for_product(
        prod, Permissions.Product_View
    )
    ...
    sla = SLA_Configuration.objects.filter(
        id=prod.sla_configuration_id
    ).first()
    benchAndPercent = []
...
def finding_querys(request, prod):
    ...
    findings = MetricsFindingFilter(
        request.GET, queryset=findings_query, pid=prod
    )
    ...
    week = end_date - timedelta(
        days=7
    )  # seven days and /newnewer are considered "new"

    # risk_acceptances = Risk_Acceptance.objects.filter(
    #     engagement__in=Engagement.objects.filter(
    #         product=prod)).prefetch_related('accepted_findings')
...
def view_product_metrics(request, pid):
    ...
    tests = tests.annotate(
        verified_finding_count=Count(
            "finding__id", filter=Q(finding__verified=True)
        )
    )
    ...
    open_objs_by_age = {
        x: len([_ for _ in filters.get("open") if _.age == x])
        for x in set([_.age for _ in filters.get("open")])
    }
    ...
    return render(
        ...
        "dojo/product_metrics.html",
        {
            ...
            "verified_objs_by_severity": sum_by_severity_level(
                filters.get("verified")
            ),
            ...
            "inactive_objs_by_severity": sum_by_severity_level(
                filters.get("inactive")
            ),
            "closed_objs": filters.get("closed", None),
            "closed_objs_by_severity": sum_by_severity_level(
                filters.get("closed")
            ),
            "false_positive_objs": filters.get("false_positive", None),
            "false_positive_objs_by_severity": sum_by_severity_level(
                filters.get("false_positive")
            ),
            "out_of_scope_objs": filters.get("out_of_scope", None),
            "out_of_scope_objs_by_severity": sum_by_severity_level(
                filters.get("out_of_scope")
            ),
            ...
            "new_objs_by_severity": sum_by_severity_level(
                filters.get("new_verified")
            ),
...
def view_engagements(request, pid):
    ...
    # prefetch only after creating the filters to avoid https://code.djangoproject.com/ticket/23771 and \
    # https://code.djangoproject.com/ticket/25375
    ...
    engs = Engagement.objects.filter(product=prod, active=False).order_by(
        "-target_end"
    )
    ...
    result_inactive_engs = get_page_items(
        request,
        inactive_engs_filter.qs,
        default_page_num,
        prefix="inactive_engs",
    )
    ...
    product_tab = Product_Tab(
        prod, title=_("All Engagements"), tab="engagements"
    )
    return render(
        request,
    ...
    engagements = (
        ...
        .annotate(
            ...
            count_findings_open_verified=Count(
                "test__finding__id",
                filter=Q(test__finding__active=True)
            ...
            count_findings_accepted=Count(
                "test__finding__id",
                filter=Q(test__finding__risk_accepted=True),
            ),
        )
    )
        ...
        if form.is_valid():
            ...
            if get_system_setting("enable_github"):
                if gform.is_valid():
                    ...
                    if (
                        github_pkey.git_conf is not None
                        and github_pkey.git_project
                    ):
                        ...
                        logger.info(
                            "Create label in repo: " + github_pkey.git_project
                        )
        ...
        if "id" in request.POST and str(product.id) == request.POST["id"]:
            ...
            if form.is_valid():
                ...
                create_notification(
                    ...
                    description=_(
                        'The product "%(name)s" was deleted by %(user)s'
                    )
...
def new_eng_for_app(request, pid, cicd=False):
    ...
    if request.method == "POST":
        form = EngForm(
            request.POST, cicd=cicd, product=product, user=request.user
        )
        ...
        if get_system_setting("enable_jira"):
            ...
            jira_project_form = JIRAProjectForm(
                target="engagement", product=product
            )
...
def new_eng_for_app_cicd(request, pid):
    # we have to use pid=pid here as new_eng_for_app expects kwargs, because that is how django calls \
    # the function based on urls.py named groups
        ...
        if form.is_valid() and (jform is None or jform.is_valid()):
            ...
            if jform and jform.is_valid():
                ...
                if new_finding.has_jira_issue:
                    ...
                    # I have no idea why, but it means we have to retrieve the \
                    # issue from JIRA to get the internal JIRA id.
...
def engagement_presets(request, pid):
    ...
    product_tab = Product_Tab(
        prod, title=_("Engagement Presets"), tab="settings"
    )
...
def edit_engagement_presets(request, pid, eid):
    ...
    product_tab = Product_Tab(
        prod, title=_("Edit Engagement Preset"), tab="settings"
    )
...
def add_engagement_presets(request, pid):
    ...
    product_tab = Product_Tab(
        prod, title=_("New Engagement Preset"), tab="settings"
    )
    return render(
        request,
...
def delete_engagement_presets(request, pid, eid):
    ...
    product_tab = Product_Tab(
        prod, title=_("Delete Engagement Preset"), tab="settings"
    )
    return render(
        request,
...
def edit_notifications(request, pid):
    ...
    if request.method == "POST":
        ...
        form = ProductNotificationsForm(
            request.POST, instance=product_notifications
        )
        # print(vars(form))
...
def add_product_member(request, pid):
                ...
                if (
                    ...
                    for user in memberform.cleaned_data["users"]:
                        ...
                        if existing_members.count() == 0:
                            ...
                            product_member.role = memberform.cleaned_data[
                                "role"
                            ]
                            product_member.save()
                messages.add_message(
    ...
    product_tab = Product_Tab(
        product, title=_("Add Product Member"), tab="settings"
    )
    return render(
        request,
...
@user_is_authorized(
    Product_Member, Permissions.Product_Manage_Members, "memberid"
)
...
@user_is_authorized(
    Product_Member, Permissions.Product_Member_Delete, "memberid"
)
...
@user_is_authorized(
    Product, Permissions.Product_API_Scan_Configuration_Add, "pid"
)
        ...
        if form.is_valid():
            ...
            try:
                ...
                if api and hasattr(api, "test_product_connection"):
                    result = api.test_product_connection(
                        product_api_scan_configuration
                    )
                    messages.add_message(
                        request,
                        messages.SUCCESS,
                        _(
                            "API connection successful with message: %(result)s."
                        )
                        % {"result": result},
                        extra_tags="alert-success",
                    )
...
def add_product_group(request, pid):
                ...
                if (
                    ...
                    for group in group_form.cleaned_data["groups"]:
                        ...
                        if groups.count() == 0:
                            ...
                            product_group.role = group_form.cleaned_data[
                                "role"
                            ]
                            product_group.save()
                messages.add_message(
    ...
    product_tab = Product_Tab(
        product, title=_("Edit Product Group"), tab="settings"
    )
    return render(
        request,
