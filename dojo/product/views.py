...
def prefetch_for_view_engagements(engs):
    ...
    if isinstance(engs, QuerySet):  # old code can arrive here with prods being a list because the query was already executed
        ...
        ACCEPTED_FINDINGS_QUERY = lambda now: Q(test__finding__risk_acceptance__isnull=False) & (Q(test__finding__risk_acceptance__expiration_date__gt=now) | Q(test__finding__risk_acceptance__expiration_date__isnull=True))
        prefetched_engs = prefetched_engs.annotate(count_findings_accepted=Count('test__finding__id', filter=ACCEPTED_FINDINGS_QUERY(timezone.now())))
