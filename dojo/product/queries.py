    ...
    if user.is_superuser or user_has_permission(user, product, permission):
        ...
        return (
            Product_Group.objects.filter(
                product=product, group__in=authorized_groups
            )
...
def get_authorized_product_members(permission):
    ...
    return Product_Member.objects.filter(product__in=products).select_related(
        "role"
    )
...
def get_authorized_product_groups(permission):
    ...
    products = get_authorized_products(permission)
    return Product_Group.objects.filter(product__in=products).select_related(
        "role"
    )
