...
def get_authorized_users_for_product_and_product_type(
    users, product, permission
):
    if users is None:
...
# Cached because it is a complex SQL query and it is called 3 times for
# the engagement lists in products
@cache_for_request
def get_authorized_users(permission, user=None):
    ...
    users = Dojo_User.objects.all().order_by(
    ...
    authorized_product_types = get_authorized_product_types(permission).values(
        "id"
    )
