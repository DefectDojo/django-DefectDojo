...
class BugcrowdAPI:
    ...
    def __init__(self, tool_config):
        ...
        if tool_config.authentication_type == "API":
            ...
            self.session.headers.update({"Authorization": "Token {}".format(self.api_token)})
    ...
    def test_connection(self):
        # Request programs
        response_programs = self.session.get(url="{}/programs".format(self.bugcrowd_api_url))
        response_programs.raise_for_status()

        # Request submissions to validate the org token
        response_subs = self.session.get(url="{}/submissions".format(self.bugcrowd_api_url))
        ...
        if response_programs.ok and response_subs.ok:
            ...
            program_names = ", ".join(list(map(lambda p: p["attributes"]["code"], progs)))
            # Request targets to validate the org token
            response_targets = self.session.get(url="{}/targets".format(self.bugcrowd_api_url))
            ...
            if response_targets.ok:
                ...
                target_names = ", ".join(list(map(lambda p: p["attributes"]["name"], targets)))
                ...
                raise Exception(
                    ...
                    "filtering, check your configuration, HTTP response was: {}".format(response_targets.text)
                )
        else:
            raise Exception(
    ...
    def test_product_connection(self, api_scan_configuration):
        ...
        return (
            ...
            and Target "{api_scan_configuration.service_key_2}" (leave service key 2 empty to get all submissions in '
            f"program)"
        )
