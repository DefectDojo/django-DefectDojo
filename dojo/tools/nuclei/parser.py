    ...
    def get_findings(self, filename, test):
        ...
        if len(data) == 0:
            ...
            for item in data:
                ...
                if info.get("severity-justification"):
                    finding.severity_justification = info.get("severity-justification")
                if item.get('mitigation'):
                    finding.mitigation = item.get('mitigation')
                if item.get('impact'):
                    finding.impact = item.get('impact')
                if item.get('line'):
                    finding.line = item.get('line')
                if item.get('file_path'):
                    finding.file_path = item.get('file_path')
