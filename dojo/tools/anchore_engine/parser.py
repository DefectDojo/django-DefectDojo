...
class AnchoreEngineParser():
    '''
    This has been updated to support the latest
    output types for Anchore engine scans.
    '''
    ...
    def get_findings(self, filename, test):
        ...
        details = f"""
        **Image hash**: {metadata.get('imageDigest',
        ...
        for item in data.get('securityEvaluation', []):
            ...
            details += f"""
            **Package type**: {item.get('packageType', 'Unknown')}\n\n"""
            ...
            if item.get('fixAvailable', 'None') != 'None':
                mitigation = f"""
                Upgrade to: {' or '.join(item['fixAvailable'].split(','))}
                \n\n"""
                mitigation += f"URL: {item.get('link', 'None')}"

            cvssv3_base_score = item.get('nvdCvssBaseScore')

            if (isinstance(cvssv3_base_score, str)
                    and cvssv3_base_score.replace('.', '', 1).isdigit()):
