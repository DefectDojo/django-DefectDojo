...
class NexposeParser(object):
    ...
    """
    def __init__(self):
        self.filter = ['vulnerable-exploited', 'vulnerable-version', 'vulnerable-potential']
    ...
    def get_findings(self, xml_output, test, **kwargs ):

        if 'filter' in kwargs.keys():
            self.filter = kwargs['filter'].split(",")
        ...
        for tests in node.findall('tests'):
            for test in tests.findall('test'):
                if test.get('id') in vulnsDefinitions and (
                        test.get('status') in self.filter):
