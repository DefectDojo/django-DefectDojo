...
def issue_r(raw_row, vuln, scan_date):
    ...
    for vuln_cat in raw_row.findall('VULNS/CAT'):
        ...
        for vuln_details in vuln_cat.findall('VULN'):
            ...
            # Extract the QDS severity from the XML
            sev = vuln_details.find('.//DETECTION/QDS').get('severity')
...
class QualysTruRiskParser(object):

    def get_scan_types(self):
        return ["Qualys Detection Data with TruRisk (XML)"]

    def get_label_for_scan_types(self, scan_type):
        return scan_type  # no custom label for now

    def get_description_for_scan_types(self, scan_type):
        return "Qualys detections with QDS Severity ratings using TruRisk."
