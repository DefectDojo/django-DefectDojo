...
class ContrastCSVParser(object):
    ...
    def __init__(self, filename, test):
        ...
        for i, row in df.iterrows():
            ...
            dupe_key = category + str(cwe) + title

            if dupe_key in dupes:
                finding = dupes[dupe_key]
                if finding.description:
                    finding.description = finding.description + "\nVulnerability ID: " + df.ix[i, 'Vulnerability ID'] + "\n" + df.ix[i, 'Vulnerability Name'] + "\n"
    ...
    def format_description(self, df, i):
        ...
        description = "Vulnerability ID: " + df.ix[i, 'Vulnerability ID'] + "\n"
        description = description + df.ix[i, 'Vulnerability Name']  + "\n\n"
        if pd.isnull(df.ix[i, 'Request Qs']) == False:
            description = description + "Request QueryString: " + str(df.ix[i, 'Request Qs']) + "\n"
        if pd.isnull(df.ix[i, 'Request Body']):
            description = description + "Request Body: " + str(df.ix[i, 'Request Body'])  + "\n"
        return description
