...
class LaceworkAPI:
    ...
    def _get_bearer_token(self) -> str:
        ...
        try:
            ...
            if not self._bearer_token:
                msg = f"Lacework token response did not contain a token. HTTP {response.status_code}. Response: {data}"
                logger.error(msg)
                raise Exception(msg)
            ...
            if expires_at_str:
                try:
                    ...
                    self._token_expiry = datetime.now(UTC) + timedelta(minutes=55)
            else:
                self._token_expiry = datetime.now(UTC) + timedelta(minutes=55)
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code == 429:
                    ...
                    logger.warning(
                        "Lacework rate limit hit. Waiting %d seconds...",
                        reset_seconds,
                    )
                ...
                if not response.ok:
                    msg = f"Lacework API error: HTTP {response.status_code} - {response.content.decode('utf-8')}"
                    logger.error(msg)
                    raise Exception(msg)
        ...
        while True:
            try:
                if current_page_url is None:
                    ...
                    if not response.ok:
                        logger.warning(
                            "Failed to fetch next page: HTTP %d",
    ...
    def test_connection(self) -> str:
        ...
        try:
            ...
            if not token:
                msg = "Failed to obtain Bearer token"
                raise Exception(msg)
        except Exception as e:
            ...
            try:
                ...
                return "Successfully connected to Lacework. Bearer token obtained successfully."
    ...
    def test_product_connection(self, api_scan_configuration) -> str:
        ...
        try:
            ...
            if repo_pattern:
                return f"Successfully connected to Lacework. Repository filter pattern: '{repo_pattern}'."
            return "Successfully connected to Lacework. No repository filter configured (will import all repositories)."
