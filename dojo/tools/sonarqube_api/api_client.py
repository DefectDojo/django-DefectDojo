...
class SonarQubeAPI:
    ...
    def find_project(self, project_name):
        ...
        response = self.session.get(
            ...
            params={"q": project_name, "qualifiers": "TRK"},
            headers={"User-Agent": "DefectDojo"},
        )
        ...
        if response.ok:
            ...
            raise Exception(
                "Unable to find the project {} due to {} - {}".format(
            ...
            if issues:
                ...
                raise Exception(
                    'Expected Issue "{}", but it returned {}.'.format(
                        issue_key, [x.get("key") for x in response.json().get("issues")]
                    )
                )
        else:
            raise Exception(
    ...
    def get_rule(self, rule_id):
        ...
        if not rule:
            ...
            if response.ok:
                ...
                raise Exception(
                    "Unable to get the rule {} due to {} - {}".format(
