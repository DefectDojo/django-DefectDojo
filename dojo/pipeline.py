...
from dojo.models import Product, Product_Member, Product_Type, Role, Dojo_Group, Dojo_Group_Member, Product_Type_Member, Global_Role
...
def search_azure_groups(kwargs, token, soc):
    ...
    if 'groups' not in kwargs['response'] or kwargs['response']['groups'] == "":
        logger.warning(
            "No groups in response. Stopping to update product type of user based on azureAD")
        return
    ...
    try:
        for group_from_response in group_IDs:
            ...
            if is_group_id(group_from_response) and group_from_response != "b3febafa-3330-4205-b40d-59cc508ef097":
                logger.debug("detected " + group_from_response +
            ...
            if settings.AZUREAD_TENANT_OAUTH2_GROUPS_FILTER == "" or re.search(settings.AZUREAD_TENANT_OAUTH2_GROUPS_FILTER, group_name):
                ...
                logger.debug("Skipping group " + group_name + " due to AZUREAD_TENANT_OAUTH2_GROUPS_FILTER " +
...
def assign_user_to_groups(user, group_names, social_provider):
    for group_name in group_names:
        group, created_group = Dojo_Group.objects.create(
            name=group_name, social_provider=social_provider)
        if created_group:
            print("Group %s for social provider %s was created", str(
        ...
        if is_member_created:
            logger.debug("User %s become member of group %s (social provider: %s)", user, str(
                group), social_provider)
...
def cleanup_old_groups_for_user(user, group_names):
    for group_member in Dojo_Group_Member.objects.select_related('group').filter(user=user):
        ...
        if str(group) not in group_names:
            logger.debug("Deleting membership of user %s from %s group %s",
                         user, group.social_provider, str(group))
            group_member.delete()


def update_product_type_azure_devops(backend, uid, user=None, social=None, *args, **kwargs):
    if settings.AZUREAD_TENANT_OAUTH2_ENABLED and settings.AZURE_DEVOPS_PERMISSION_AUTO_IMPORT and isinstance(backend, AzureADTenantOAuth2):
        soc = user.social_auth.get()
        token = soc.extra_data['access_token']
        group_names = search_azure_groups(kwargs, token, soc)
        logger.debug("detected groups " + str(group_names))
        if len(group_names) > 0:
            userLogin = kwargs['response']['email']
            request_headers = {'Authorization': 'Bearer ' + token}
            graph_user_request = requests.get(
                (str(soc.extra_data['resource']) + '/v1.0/users/' + userLogin), headers=request_headers)
            graph_user_request.raise_for_status()
            graph_user_request_json = graph_user_request.json()
            jobTitle = graph_user_request_json['jobTitle']
            officeLocation = graph_user_request_json['officeLocation']
            logger.debug("detected jobTitle " + jobTitle +
                         " and officeLocation " + officeLocation)
            ...
            result_query_subjects = graph_client.query_subjects({
                "query": userLogin,
                "subjectKind": ["User"]
            })
            ...
            if result_query_subjects is not None and settings.AZURE_DEVOPS_MAIN_SECURITY_GROUP in group_names:
                ...
                if officeLocation in settings.AZURE_DEVOPS_OFFICES_LOCATION.split(",")[0] and jobTitle != settings.AZURE_DEVOPS_JOBS_TITLE.split(",")[0]:
                    Global_Role.objects.get_or_create(
                        user=user, role=Role.objects.get(id=Roles.Maintainer))
                elif officeLocation in settings.AZURE_DEVOPS_OFFICES_LOCATION.split(",")[1]:
                    Global_Role.objects.get_or_create(
                        user=user, role=Role.objects.get(id=Roles.Reader))
                else:
                    roleAssigned = {
                        'role': Role.objects.get(id=Roles.Developer)}
                    if jobTitle in settings.AZURE_DEVOPS_JOBS_TITLE.split(",")[0]:
                        roleAssigned = {
                            'role': Role.objects.get(id=Roles.Leader)}

                    # Get user's product type
                    result_memberships = graph_client.get_membership(
                        result_query_subjects[0].descriptor, None)

                    group_team_leve1 = custom_filter_group(
                        result_memberships.additional_properties['value'], graph_client, settings.AZURE_DEVOPS_GROUP_TEAM_FILTERS.split("//")[0])

                    group_team_leve2 = custom_filter_group(graph_client.get_membership(
                        group_team_leve1.descriptor, None).additional_properties['value'], graph_client, settings.AZURE_DEVOPS_GROUP_TEAM_FILTERS.split("//")[1])
                    ...
                    if group_team_leve2 not in user_product_types_names:
                        try:
                            ...
                            product_type = Product_Type.objects.get(
                                name=group_team_leve2.display_name)
                        ...
                        Product_Type_Member.objects.get_or_create(
                            product_type=product_type, user=user, defaults=roleAssigned)
                        logger.debug(
                            "User %s become member of product type %s", user, product_type.name)
                        ...
                        for product_type_name in user_product_types_names:
                            if product_type_name != group_team_leve2.display_name:
                                ...
                                Product_Type_Member.objects.filter(
                                    product_type=product_type, user=user).delete()
                                logger.debug("Deleting membership of user %s from product type %s",
                                             user, product_type_name)


def custom_filter_group(result, graph_client, regex):
    for member in result:
        result_get_group = graph_client.get_group(
            member['containerDescriptor'])
...
def update_product_access(backend, uid, user=None, social=None, *args, **kwargs):
    if settings.GITLAB_PROJECT_AUTO_IMPORT is True:
        ...
        user_product_names = [prod.name for prod in get_authorized_products(
            Permissions.Product_View, user)]
        # Get Gitlab access token
        soc = user.social_auth.get()
        token = soc.extra_data['access_token']
        # Get user's projects list on Gitlab
        gl = gitlab.Gitlab(
            settings.SOCIAL_AUTH_GITLAB_API_URL, oauth_token=token)
            ...
            if project.path_with_namespace not in user_product_names:
                ...
                if settings.GITLAB_PROJECT_IMPORT_URL:
                    if hasattr(project, 'web_url') and len(project.web_url) > 0:
                        product.description = "[" + project.web_url + \
                            "](" + project.web_url + ")"
...
def sanitize_username(username):
    ...
    allowed_chars = filter(
        lambda char: allowed_chars_regex.match(char), list(username))
