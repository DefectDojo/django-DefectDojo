...
def edit_cred_product(request, pid, ttid):
    ...
    product_tab = Product_Tab(
        prod, title="Edit Product Credential", tab="settings"
    )
    return render(
        request,
...
def edit_cred_product_engagement(request, eid, ttid):
    ...
    return render(
        request,
        "dojo/edit_cred_all.html",
        {"tform": tform, "cred_type": "Engagement"},
    )
...
def new_cred_product(request, pid):
    ...
    if request.method == "POST":
        ...
        if tform.is_valid():
            # Select the credential mapping object from the selected list and
            # only allow if the credential is associated with the product
...
def new_cred_product_engagement(request, eid):
    ...
    if request.method == "POST":
        ...
        if tform.is_valid() and tform.cleaned_data["cred_user"]:
            ...
            if not cred_user:
                message = (
                    "Credential must first be associated with this product."
                )
...
def delete_cred_controller(request, destination_url, id, ttid):
    ...
    if request.method == "POST":
        ...
        if destination_url == "cred":
            ...
            cred_lookup = Cred_Mapping.objects.filter(
                cred_id=cred.cred_id
            ).exclude(engagement__isnull=True)
