...
DEFAULT_STATS = {
    sev.lower(): {stat_field: 0 for stat_field in STATS_FIELDS}
...
class UniqueUploadNameProvider:
    ...
    def __call__(self, model_instance, filename):
        ...
        filename = (
            "%s_%s" % (base, uuid4()) if self.keep_basename else str(uuid4())
        )
        if self.keep_ext:
            filename += ext
...
class Dojo_User(User):
    ...
    def wants_block_execution(user):
        # this return False if there is no user, i.e. in celery processes,
        # unittests, etc.
        return (
            hasattr(user, "usercontactinfo")
            and user.usercontactinfo.block_execution
        )
...
class UserContactInfo(models.Model):
    ...
    block_execution = models.BooleanField(
        ...
        help_text=_(
            (
                "Instead of async deduping a finding "
                "the findings will be deduped synchronously "
...
class System_Settings(models.Model):
    ...
    max_dupes = models.IntegerField(
        ...
        help_text=_(
            ...
            "oldest will be deleted. Duplicate will not be deleted when left empty. "
            "A value of 0 will remove all duplicates."
        ),
    )
    ...
    enable_jira_web_hook = models.BooleanField(
        ...
        help_text=_(
            ...
            "to use a secret below and / or IP whitelist "
            "the JIRA server using a proxy such as Nginx."
        ),
        blank=False,
    )
    ...
    disable_jira_webhook_secret = models.BooleanField(
        ...
        help_text=_(
            "Allows incoming requests without a secret "
            "(discouraged legacy behaviour)"
        ),
        blank=False,
    )
    ...
    mail_notifications_to = models.CharField(
    ...
    false_positive_history = models.BooleanField(
        ...
        help_text=_(
            (
                "DefectDojo will automatically mark the finding as a false positive "
                "if the finding has been previously marked as a false positive. "
                "Not needed when using deduplication, advised to not combine these two."
            )
        ),
    )
    ...
    url_prefix = models.CharField(
        ...
        blank=True,
        help_text=_(
            "URL prefix if DefectDojo is installed "
            "in it's own virtual subdirectory."
        ),
    )
    ...
    enable_template_match = models.BooleanField(
        ...
        help_text=_(
            "Enables global remediation advice and matching on CWE and Title. "
            "The text will be replaced for mitigation, "
            "impact and references on a finding. "
            "Useful for providing consistent impact "
    ...
    engagement_auto_close = models.BooleanField(
        ...
        help_text=_(
            "Closes an engagement after 3 days (default) "
            "past due date including last update."
        ),
    )
    ...
    engagement_auto_close_days = models.IntegerField(
        ...
        help_text=_(
            "Closes an engagement after the specified number of days "
            "past due date including last update."
        ),
    )
    ...
    enable_notify_sla_active = models.BooleanField(
        ...
        help_text=_(
            "Enables Notify when time to remediate according "
            "to Finding SLA's is breached for active Findings."
        ),
    )
    ...
    enable_notify_sla_active_verified = models.BooleanField(
        ...
        blank=False,
        verbose_name=_(
            "Enable Notify SLA's Breach for active, verified Findings"
        ),
        help_text=_(
            "Enables Notify when time to remediate according "
            "to Finding SLA's is breached for active, verified Findings."
        ),
    )
    ...
    enable_notify_sla_jira_only = models.BooleanField(
        ...
        blank=False,
        verbose_name=_(
            "Enable Notify SLA's Breach for Findings linked to JIRA"
        ),
        help_text=_(
            "Enables Notify when time to remediate according "
            "to Finding SLA's is breached for Findings "
            "that are linked to JIRA issues."
        ),
    )
    ...
    enable_notify_sla_exponential_backoff = models.BooleanField(
        ...
        blank=False,
        verbose_name=_(
            "Enable an exponential backoff strategy "
            "for SLA breach notifications."
        ),
        help_text=_(
            "Enable an exponential backoff strategy for SLA breach notifications, "
            "e.g. 1, 2, 4, 8, etc. Otherwise it alerts every day"
        ),
    )
    ...
    default_group_email_pattern = models.CharField(
        ...
        blank=True,
        help_text=_(
            "New users will only be assigned to the default group, "
            "when their email address matches this regex pattern. This is optional condition."
        ),
    )
...
class FileUpload(models.Model):
    ...
    file = models.FileField(
        upload_to=UniqueUploadNameProvider("uploaded_files")
    )
...
class Product_Type(models.Model):
    """Product types represent the top level model, these can be business unit divisions,
    different offices or locations, development teams,
    or any other logical way of distinguishing “types” of products.
    `
           Examples:
             * IAM Team
             * Internal / 3rd Party
...
class DojoMeta(models.Model):
    ...
    def clean(self):
        ...
        if ids_count > 1:
            raise ValidationError(
                "Metadata entries may not have more than one relation, "
                "either a product, an endpoint either or a finding"
            )
...
class Product(models.Model):
    ...
    members = models.ManyToManyField(
        Dojo_User,
        through="Product_Member",
        related_name="product_members",
        blank=True,
    )
    authorization_groups = models.ManyToManyField(
        Dojo_Group,
        through="Product_Group",
    ...
    def findings_count(self):
        try:
            ...
            # ideally it's always prefetched and we can remove this code in the
            # future
    ...
    # only used in APIv2 serializers.py, query should be aligned with
    # findings_count
    @cached_property
...
class ToolConfigForm_Admin(forms.ModelForm):
    ...
    # django doesn't seem to have an easy way to handle password fields as
    # PasswordInput requires reentry of passwords
...
class Engagement_Presets(models.Model):
    title = models.CharField(
        max_length=500,
        default=None,
        help_text=_("Brief description of preset."),
    )
...
class Engagement(models.Model):
    ...
    progress = models.CharField(
        max_length=100, default="threat_model", editable=False
    )
    ...
    deduplication_on_engagement = models.BooleanField(
        ...
        help_text=_(
            "If enabled deduplication will only mark a finding "
            "in this engagement as duplicate of another finding if "
            "both findings are in this engagement. If disabled, "
            "deduplication is on the product level."
        ),
    )
...
class Endpoint_Status(models.Model):
    ...
    mitigated_time = models.DateTimeField(
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["finding", "endpoint"],
                name="endpoint-finding relation",
            )
        ]
...
class Endpoint(models.Model):
    ...
    def clean(self):
        ...
        if self.protocol or self.protocol == "":
            if not re.match(
                ...
                errors.append(
                    ValidationError(
                        'Protocol "{}" has invalid format'.format(
                            self.protocol
                        )
                    )
                )
        ...
        if self.userinfo or self.userinfo == "":
            if not re.match(
                ...
                errors.append(
                    ValidationError(
                        'Userinfo "{}" has invalid format'.format(
                            self.userinfo
                        )
                    )
                )
        ...
        if self.host:
            if not re.match(
                r"^[A-Za-z0-9_\-\+][A-Za-z0-9_\.\-\+]+$", self.host
            ):
                try:
        ...
        if self.path or self.path == "":
            ...
            if any([null_char in self.path for null_char in null_char_list]):
                ...
                if "postgres" in db_type:
                    ...
                    errors.append(
                        ValidationError(
                            'Path "{}" has invalid format - It contains the NULL character. \
                                The following action was taken: {}'.format(
        ...
        if self.query or self.query == "":
            ...
            if any([null_char in self.query for null_char in null_char_list]):
                ...
                if "postgres" in db_type:
                    ...
                    errors.append(
                        ValidationError(
                            'Query "{}" has invalid format - It contains the NULL character. \
                                The following action was taken: {}'.format(
                                old_value, action_string
                            )
                        )
        ...
        if self.fragment or self.fragment == "":
            ...
            if any(
                [null_char in self.fragment for null_char in null_char_list]
            ):
                ...
                if "postgres" in db_type:
                    ...
                    errors.append(
                        ValidationError(
                            'Fragment "{}" has invalid format - It contains the NULL character. \
                                The following action was taken: {}'.format(
                                old_value, action_string
                            )
                        )
        ...
        try:
            if self.host:
                ...
                if not self.protocol:
                    if clean_url[: len(dummy_scheme) + 3] == (
                        dummy_scheme + "://"
                    ):
    ...
    def host_endpoints(self):
        return Endpoint.objects.filter(
            host=self.host, product=self.product
        ).distinct()
    ...
    def from_uri(uri):
        ...
        userinfo = (
            ":".join(url.userinfo) if url.userinfo not in [(), ("",)] else None
        )
...
class Sonarqube_Issue(models.Model):
    ...
    status = models.CharField(
        max_length=20, help_text=_("SonarQube issue status")
    )
...
class Test(models.Model):
    engagement = models.ForeignKey(
        Engagement, editable=False, on_delete=models.CASCADE
    )
    lead = models.ForeignKey(
        Dojo_User,
        editable=True,
    ...
    def get_breadcrumbs(self):
        ...
        bc += [
            {"title": str(self), "url": reverse("view_test", args=(self.id,))}
        ]
        return bc
    ...
    def deduplication_algorithm(self):
        ...
        if hasattr(settings, "DEDUPLICATION_ALGORITHM_PER_PARSER"):
            if (
                self.test_type.name
                in settings.DEDUPLICATION_ALGORITHM_PER_PARSER
            ):
                ...
                deduplicationAlgorithm = (
                    settings.DEDUPLICATION_ALGORITHM_PER_PARSER[self.scan_type]
                )
        else:
            deduplicationLogger.debug(
    ...
    def hash_code_fields(self):
        ...
        deduplicationLogger.debug(
            f"HASHCODE_FIELDS_PER_SCANNER is: {hashCodeFields}"
        )
...
class Test_Import(TimeStampedModel):
    ...
    type = models.CharField(
        max_length=64, null=False, blank=False, default="unknown"
    )
    ...
    def get_queryset(self):
        ...
        super_query = super_query.annotate(
            closed_findings_count=Count(
                "findings",
                filter=Q(
                    test_import_finding_action__action=IMPORT_CLOSED_FINDING
                ),
            )
        )
        super_query = super_query.annotate(
        ...
        super_query = super_query.annotate(
            untouched_findings_count=Count(
                "findings",
                filter=Q(
                    test_import_finding_action__action=IMPORT_UNTOUCHED_FINDING
                ),
            )
        )
        return super_query
...
class Finding(models.Model):
    ...
    sla_start_date = models.DateField(
        ...
        help_text=_(
            "(readonly)The date used as start date for SLA calculation. "
            "Set by expiring risk acceptances. Empty by default, "
            "causing a fallback to 'date'."
        ),
    )
    ...
    cve = models.CharField(
        ...
        help_text=_(
            "An id of a vulnerability in a security advisory associated with this finding. "
            "Can be a Common Vulnerabilities and Exposures (CVE) or from other sources."
        ),
    )
    ...
    cvssv3_score = models.FloatField(
        ...
        help_text=_(
            "Numerical CVSSv3 score for the vulnerability. "
            "If the vector is given, the score is updated while saving the finding"
        ),
    )
    ...
    # in defectdojo verified means: "we have verified the finding and it turns
    # out that it's not a false positive"
    ...
    nb_occurences = models.IntegerField(
        ...
        help_text=_(
            "Number of occurences in the source tool when several vulnerabilites "
            "were found and aggregated by the scanner."
        ),
    )
    """
    this is useful for vulnerabilities on dependencies :
    helps answer the question "Did I add this vulnerability or was it discovered recently?"
    """
    ...
    # The service is used to generate the hash_code, so that it gets part of
    # the deduplication of findings.
    service = models.CharField(
        ...
        help_text=_(
            "A service is a self-contained piece of functionality within a Product. "
            "This is an optional field which is used in deduplication of findings when set."
        ),
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            # models.Index(fields=['file_path']), # can't add index because the
            # field has max length 4000.
    ...
    def compute_hash_code(self):
        ...
        # Check if all elements of HASHCODE_FIELDS_PER_SCANNER are in
        # HASHCODE_ALLOWED_FIELDS
        if not (
            all(
                elem in settings.HASHCODE_ALLOWED_FIELDS
                for elem in hash_code_fields
            )
        ):
            deduplicationLogger.debug(
                "compute_hash_code - configuration error: "
                "some elements of HASHCODE_FIELDS_PER_SCANNER are not in the allowed "
                "list HASHCODE_ALLOWED_FIELDS. "
        ...
        for hashcodeField in hash_code_fields:
            if hashcodeField == "endpoints":
                ...
                deduplicationLogger.debug(
                    hashcodeField + " : " + my_vulnerability_ids
                )
            else:
                # Generically use the finding attribute having the same name,
                # converts to str in case it's integer
                fields_to_hash = fields_to_hash + str(
                    getattr(self, hashcodeField)
                )
                deduplicationLogger.debug(
    ...
    def get_vulnerability_ids(self):
        ...
        if self.id is None:
            if self.unsaved_vulnerability_ids:
                ...
                # convert list of unsaved vulnerability_ids to the list of
                ...
                # deduplicate (usually done upon saving finding) and sort
                # endpoints
            ...
            # convert list of vulnerability_ids to the list of their canonical
            # representation
            vulnerability_id_str_list = list(
                map(
                    lambda vulnerability_id: str(vulnerability_id),
    ...
    def get_endpoints(self):
        ...
        if self.id is None:
            if len(self.unsaved_endpoints) > 0:
                deduplicationLogger.debug(
                    "get_endpoints before the finding was saved"
                )
                # convert list of unsaved endpoints to the list of their
                ...
                # endpoints
                endpoint_str = "".join(
                    sorted(list(dict.fromkeys(endpoint_str_list)))
                )
            else:
                """
                ...
                After saving dynamic_finding may be set to False probably during the saving process
                (observed on Bandit scan before forcing dynamic_finding=False at parser level)
                """
                deduplicationLogger.debug(
                    "trying to get endpoints on a finding before it was saved but no endpoints found "
                    "(static parser wrongly identified as dynamic?"
                )
        else:
            deduplicationLogger.debug(
            ...
            # convert list of endpoints to the list of their canonical
            # representation
            endpoint_str_list = list(
    ...
    def get_scanner_confidence_text(self):
        if self.scanner_confidence and isinstance(
            self.scanner_confidence, int
        ):
    ...
    def github_conf_new(self):
        try:
            return self.test.engagement.product.github_pkey_set.all()[
                0
            ].git_conf
        except Exception:
            return None
    ...
    def save(
        ...
        **kwargs,
        ...
        """
        Finding.save is called once from serializers.py with dedupe_option=False
        because the finding is not ready yet, for example the endpoints are not built
        It is then called a second time with dedupe_option defaulted to true;
        now we can compute the hash_code and run the deduplication
        """
        if dedupe_option:
            if self.hash_code is not None:
                deduplicationLogger.debug(
                    "Hash_code already computed for finding"
                )
            else:
        ...
        if self.pk is None:
            ...
            if (self.file_path is not None) and (
                ...
                self.static_finding = True
            """
            because we have reduced the number of (super()).save() calls,
            the helper is no longer called for new findings
        ...
        """
        only perform post processing (in celery task) if needed.
        this check avoids submitting 1000s of tasks to celery that will do nothing
        """
        if (
            dedupe_option
            or false_history
            ...
            logger.debug(
                "no options selected that require finding post processing"
            )

    # Check if a mandatory field is empty. If it's the case, fill it with "no
    # <fieldName> given"
    def clean(self):
    ...
    def get_file_path_with_raw_link(self):
        ...
        if (
            self.test.engagement.source_code_management_uri is not None
            and "https://github.com/"
            in self.test.engagement.source_code_management_uri
        ):
            if self.test.commit_hash:
                ...
                link += (
                    "/blob/"
                    + self.test.engagement.commit_hash
                    + "/"
                    + self.file_path
                )
                ...
                link += (
                    "/blob/"
                    + self.test.engagement.branch_tag
                    + "/"
                    + self.file_path
                )
            else:
    ...
    def get_references_with_links(self):
        ...
        for match in matches:
            ...
            # Only replace the same matches one time, otherwise the links will
            # be corrupted
            if (
    ...
    def vulnerability_ids(self):
        ...
        if vulnerability_ids and self.cve:
            # Make sure the first entry of the list is the value of the cve
            # field
...
class Vulnerability_Id(models.Model):
    finding = models.ForeignKey(
        Finding, editable=False, on_delete=models.CASCADE
    )
...
class Finding_Group(TimeStampedModel):
    ...
    def mitigated(self):
        return all(
            [find.mitigated is not None for find in self.findings.all()]
        )
...
class Finding_Template(models.Model):
    ...
    cvssv3 = models.TextField(
        validators=[cvssv3_regex], max_length=117, null=True
    )
    ...
    template_match = models.BooleanField(
        ...
        help_text=_(
            "Enables this template for matching remediation advice. "
            "Match will be applied to all active, verified findings by CWE."
        ),
    )
    ...
    tags = TagField(
        ...
        force_lowercase=True,
        help_text=_(
            "Add tags that help describe this finding template. "
        ...
        bc += [
            {
                "title": "Check List",
                "url": reverse(
                    "complete_checklist", args=(self.engagement.id,)
                ),
            }
        ]
        return bc
...
class Risk_Acceptance(models.Model):
    ...
    name = models.CharField(
        ...
        blank=False,
        help_text=_(
            "Descriptive name which in the future may also be used to group risk acceptances "
    ...
    owner = models.ForeignKey(
        ...
        on_delete=models.RESTRICT,
        help_text=_(
            "User in DefectDojo owning this acceptance. "
            "Only the owner and staff users can edit the risk acceptance."
        ),
    )
...
class Announcement(models.Model):
    message = models.CharField(
        max_length=500,
        help_text=_(
            "This dismissable message will be displayed on all pages for authenticated users. "
            "It can contain basic html tags, for example "
            "<a href='https://www.fred.com' style='color: #337ab7;' target='_blank'>https://example.com</a>"
        ),
        default="",
    )
    ...
    style = models.CharField(
        max_length=64,
        choices=ANNOUNCEMENT_STYLE_CHOICES,
        default="info",
        help_text=_(
            "The style of banner to display. (info, success, warning, danger)"
        ),
    )
...
class BannerConf(models.Model):
    ...
    banner_message = models.CharField(
        max_length=500,
        help_text=_(
            "This message will be displayed on the login page. It can contain basic html tags, "
            "for example <a href='https://www.fred.com' style='color: #337ab7;' target='_blank'>https://example.com</a>"
        ),
        default="",
    )
...
class GITHUB_Issue(models.Model):
    ...
    issue_url = models.URLField(
        max_length=2000, verbose_name=_("GitHub issue URL")
    )
    ...
    def __str__(self):
        return (
            str(self.issue_id) + "| GitHub Issue URL: " + str(self.issue_url)
        )
...
class JIRA_Instance(models.Model):
    ...
    issue_template_dir = models.CharField(
        ...
        blank=True,
        help_text=_(
            "Choose the folder containing the Django templates used to render the JIRA issue description. "
            "These are stored in dojo/templates/issue-trackers. Leave empty to use the default jira_full templates."
        ),
    )
    epic_name_id = models.IntegerField(
        help_text=_(
            "To obtain the 'Epic name id' visit "
            "https://<YOUR JIRA URL>/rest/api/2/field and search for Epic Name. "
            "Copy the number out of cf[number] and paste it here."
        )
    )
    open_status_key = models.IntegerField(
        verbose_name=_("Reopen Transition ID"),
        help_text=_(
            "Transition ID to Re-Open JIRA issues, visit "
    ...
    close_status_key = models.IntegerField(
        ...
        help_text=_(
            "Transition ID to Close JIRA issues, visit "
            "https://<YOUR JIRA URL>/rest/api/latest/issue/<ANY VALID ISSUE KEY>/transitions?expand=transitions.fields "
            "to find the ID for your JIRA instance"
        ),
    )
    ...
    finding_text = models.TextField(
        ...
        blank=True,
        help_text=_(
            "Additional text that will be added to the finding in Jira. "
            "For example including how the finding was created or who to contact for more information."
        ),
    )
    ...
    finding_jira_sync = models.BooleanField(
        default=False,
        blank=False,
        verbose_name=_("Automatically sync Findings with JIRA?"),
        help_text=_(
    ...
    def __str__(self):
        return (
            self.configuration_name + " | " + self.url + " | " + self.username
        )
...
class JIRA_Project(models.Model):
    ...
    push_all_issues = models.BooleanField(
        ...
        blank=True,
        help_text=_(
            "Automatically maintain parity with JIRA. "
            "Always create and update JIRA tickets for findings in this Product."
        ),
    )
    enable_engagement_epic_mapping = models.BooleanField(
    ...
    product_jira_sla_notification = models.BooleanField(
        default=False,
        blank=True,
        verbose_name=_("Send SLA notifications as comment?"),
    )
...
class JIRA_Issue(models.Model):
    jira_project = models.ForeignKey(
        JIRA_Project, on_delete=models.CASCADE, null=True
    )
    ...
    def set_obj(self, obj):
        if isinstance(obj, Finding):
            ...
            raise ValueError(
                "unknown object type while creating JIRA_Issue: %s"
                % to_str_typed(obj)
            )
...
class Notifications(models.Model):
    ...
    jira_update = MultiSelectField(
        ...
        help_text=_(
            "JIRA sync happens in the background, "
            "errors will be shown as notifications/alerts so make sure to subscribe"
        ),
    )
    ...
    def merge_notifications_list(cls, notifications_list):
        ...
        for notifications in notifications_list:
            if result is None:
                """
                we start by copying the first instance, because creating a new instance
                would set all notification columns to 'alert' :-()
                """
                ...
                result.upcoming_engagement = merge_sets_safe(
                    result.upcoming_engagement,
                    notifications.upcoming_engagement,
                )
                ...
                result.auto_close_engagement = merge_sets_safe(
                    ...
                    notifications.auto_close_engagement,
                )
                ...
                result.other = merge_sets_safe(
                    result.other, notifications.other
                )
...
class Tool_Product_History(models.Model):
    ...
    succesfull = models.BooleanField(
        default=True, verbose_name=_("Succesfully")
    )
    configuration_details = models.CharField(
...
class Cred_User(models.Model):
    ...
    is_valid = models.BooleanField(
        default=True, verbose_name=_("Login is valid")
    )
...
class App_Analysis(models.Model):
    ...
    name = models.CharField(max_length=200, null=False)
    user = models.ForeignKey(
        Dojo_User, editable=True, on_delete=models.RESTRICT
    )
...
class Testing_Guide(models.Model):
    ...
    identifier = models.CharField(
        ...
        blank=True,
        null=True,
        help_text=_("Test Unique Identifier"),
    )
    name = models.CharField(max_length=400, help_text=_("Name of the test"))
    summary = models.CharField(
        max_length=800, help_text=_("Summary of the test")
    )
    objective = models.CharField(
        max_length=800, help_text=_("Objective of the test")
    )
...
class Benchmark_Product(models.Model):
    ...
    control = models.ForeignKey(
        Benchmark_Requirement, on_delete=models.CASCADE
    )
...
class Benchmark_Product_Summary(models.Model):
    ...
    benchmark_type = models.ForeignKey(
        Benchmark_Type, on_delete=models.CASCADE
    )
    asvs_level = (
        ("Level 1", "Level 1"),
        ("Level 2", "Level 2"),
...
class Question(PolymorphicModel, TimeStampedModel):
    ...
    order = models.PositiveIntegerField(
        default=1, help_text=_("The render order")
    )
    ...
    text = models.TextField(
        blank=False, help_text=_("The question text"), default=""
    )
...
class ChoiceQuestion(Question):
    ...
    multichoice = models.BooleanField(
        default=False, help_text=_("Select one or more")
    )
...
class TextAnswer(Answer):
    answer = models.TextField(
        blank=False, help_text=_("The answer text"), default=""
    )
