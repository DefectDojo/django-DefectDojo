...
def critical_product_metrics(request, mtype):
    ...
    add_breadcrumb(
        title=page_name, top_level=not len(request.GET), request=request
    )
    return render(
        request,
        template,
...
def finding_querys(prod_type, request):
    ...
    if not findings_qs and not findings_query:
        ...
        findings_qs = (
            findings if isinstance(findings, QuerySet) else findings.qs
        )
    ...
    # it is validated according to https://www.flake8rules.com/rules/E722.html
    except Exception:
...
def endpoint_querys(prod_type, request):
    ...
    if not endpoints_qs:
        ...
        endpoints_qs = (
            endpoints if isinstance(endpoints, QuerySet) else endpoints.qs
        )
        messages.add_message(
            request,
            messages.ERROR,
...
def metrics(request, mtype):
    ...
    return render(
        ...
        template,
        {
            ...
            "accepted_per_month": filters["monthly_counts"][
                "accepted_per_period"
            ],
            "accepted_per_week": filters["weekly_counts"][
                "accepted_per_period"
            ],
...
def product_type_counts(request):
    ...
    add_breadcrumb(
        title=_("Bi-Weekly Metrics"), top_level=True, request=request
    )
...
def engineer_metrics(request):
    ...
    add_breadcrumb(
        title=_("Engineer Metrics"), top_level=True, request=request
    )
...
def view_engineer(request, eid):
    ...
    if not (
        request.user.is_superuser or request.user.username == user.username
    ):
    ...
    findz = findings.filter(
        mitigated__isnull=True, active=True, risk_acceptance=None
    )
