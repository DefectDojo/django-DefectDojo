...
def delete_issue(request, id, page, objid):
    ...
    if (
        page is None
        or str(request.user) != note.author.username
    ...
    if form.is_valid():
        ...
        messages.add_message(
            request, messages.SUCCESS, "Note deleted.", extra_tags="alert-success"
        )
    else:
        messages.add_message(
            request,
...
def edit_issue(request, id, page, objid):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            form = NoteForm()
            messages.add_message(
                request, messages.SUCCESS, "Note edited.", extra_tags="alert-success"
            )
            return HttpResponseRedirect(reverse(reverse_url, args=(object_id,)))
        else:
            messages.add_message(
