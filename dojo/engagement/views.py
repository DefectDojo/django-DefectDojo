...
def get_filtered_engagements(request, view):
    ...
    engagements = engagements.select_related("product", "product__prod_type").prefetch_related(
        "lead", "tags", "product__tags"
    )
...
def engagements(request, view):
    ...
    engagement_name_words = sorted(
        get_authorized_engagements(Permissions.Engagement_View).values_list("name", flat=True).distinct()
    )

    add_breadcrumb(title=f"{view.capitalize()} Engagements", top_level=not len(request.GET), request=request)
...
def engagements_all(request):
    ...
    filter_string_matching = get_system_setting("filter_string_matching", False)
    products_filter_class = (
        ProductEngagementsFilterWithoutObjectLookups if filter_string_matching else ProductEngagementsFilter
    )
    test_count_subquery = build_count_subquery(
        Test.objects.filter(engagement=OuterRef("pk")),
        group_field="engagement_id",
    ...
    add_breadcrumb(title="All Engagements", top_level=not len(request.GET), request=request)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.add_message(
                request, messages.SUCCESS, "Engagement updated successfully.", extra_tags="alert-success"
            )

            success, jira_project_form = jira_helper.process_jira_project_form(
                request, instance=jira_project, target="engagement", engagement=engagement, product=engagement.product
            )
            error = not success
...
def copy_engagement(request, eid):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.add_message(
                request, messages.SUCCESS, "Engagement Copied successfully.", extra_tags="alert-success"
            )
            create_notification(
                event="engagement_copied",  # TODO: - if 'copy' functionality will be supported by API as well, 'create_notification' needs to be migrated to place where it will be able to cover actions from both interfaces
        ...
        messages.add_message(
            request, messages.ERROR, "Unable to copy engagement, please try again.", extra_tags="alert-danger"
        )
...
class ViewEngagement(View):
    ...
    def post(self, request, eid, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            messages.add_message(request, messages.SUCCESS, "Note added successfully.", extra_tags="alert-success")
        creds = Cred_Mapping.objects.filter(product=eng.product).select_related("cred_id").order_by("cred_id")
        cred_eng = Cred_Mapping.objects.filter(engagement=eng.id).select_related("cred_id").order_by("cred_id")
...
def prefetch_for_view_tests(tests):
    ...
    return prefetched.annotate(
        ...
        count_findings_test_active_fix_available=Coalesce(
            count_subquery(base_findings.filter(active=True, fix_available=True)),
            Value(0),
        ),
...
def add_tests(request, eid):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.add_message(request, messages.SUCCESS, "Test added successfully.", extra_tags="alert-success")
...
def close_eng(request, eid):
    ...
    messages.add_message(request, messages.SUCCESS, "Engagement closed successfully.", extra_tags="alert-success")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                cl.save()
                form.save_m2m()
            messages.add_message(request, messages.SUCCESS, "Checklist saved.", extra_tags="alert-success")
...
def add_risk_acceptance(request, eid, fid=None):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if form.cleaned_data["notes"]:
                notes = Notes(entry=form.cleaned_data["notes"], author=request.user, date=timezone.now())
                notes.save()
            ...
            messages.add_message(request, messages.SUCCESS, "Risk acceptance saved.", extra_tags="alert-success")
    ...
    finding_choices = (
        ...
        .prefetch_related("test", "finding_group_set")
        .order_by("test__id", "numerical_severity", "title")
    )
    ...
    if fid:
        ...
        form.fields["accepted_findings"].label_from_instance = (
            lambda obj: f"({obj.test.scan_type}) - ({obj.severity}) - {obj.title} - {obj.date} - {obj.status()} - {obj.finding_group})"
        )
    ...
    if request.method == "POST":
        ...
        if "decision" in request.POST:
            ...
            if not errors:
                ...
                messages.add_message(
                    request, messages.SUCCESS, "Risk Acceptance saved successfully.", extra_tags="alert-success"
                )
        ...
        if "delete_note" in request.POST:
            ...
            if note.author.username == request.user.username:
                ...
                note.delete()
                messages.add_message(
                    request, messages.SUCCESS, "Note deleted successfully.", extra_tags="alert-success"
                )
            else:
                messages.add_message(
                    request,
        ...
        if "replace_file" in request.POST:
            replace_form = ReplaceRiskAcceptanceProofForm(request.POST, request.FILES, instance=risk_acceptance)
            ...
            if not errors:
                ...
                messages.add_message(
                    request, messages.SUCCESS, "New Proof uploaded successfully.", extra_tags="alert-success"
                )
            else:
                logger.error(replace_form.errors)

        if "add_findings" in request.POST:
            add_findings_form = AddFindingsRiskAcceptanceForm(request.POST, request.FILES, instance=risk_acceptance)
...
def delete_risk_acceptance(request, eid, raid):
    ...
    messages.add_message(request, messages.SUCCESS, "Risk acceptance deleted successfully.", extra_tags="alert-success")
    return HttpResponseRedirect(reverse("view_engagement", args=(eng.id,)))
...
def download_risk_acceptance(request, eid, raid):
    ...
    response = StreamingHttpResponse(
        FileIterWrapper((Path(settings.MEDIA_ROOT) / "risk_acceptance.path.name").open(mode="rb"))
    )
...
def upload_threatmodel(request, eid):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            eng.save()
            messages.add_message(request, messages.SUCCESS, "Threat model saved.", extra_tags="alert-success")
            return HttpResponseRedirect(reverse("view_engagement", args=(eid,)))
    else:
