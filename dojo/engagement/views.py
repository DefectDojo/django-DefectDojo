...
def engagements(request, view):
    ...
    product_name_words = sorted(
        get_authorized_products(Permissions.Product_View).values_list(
            "name", flat=True
        )
    )
...
def engagements_all(request):
    products_with_engagements = get_authorized_products(
        Permissions.Engagement_View
    )
    ...
    # count using prefetch instead of just using 'engagement__set_test_test`
    # to avoid loading all test in memory just to count them
    filter_qs = products_with_engagements.prefetch_related(
        Prefetch(
            "engagement_set",
            queryset=Engagement.objects.all().annotate(
                test_count=Count("test__id")
            ),
        )
    )
...
def edit_engagement(request, eid):
    ...
    product_tab = Product_Tab(
        engagement.product, title=title, tab="engagements"
    )
...
def delete_engagement(request, eid):
    ...
    product_tab = Product_Tab(
        product, title="Delete Engagement", tab="engagements"
    )
    product_tab.setEngagement(engagement)
    return render(
        request,
...
def copy_engagement(request, eid):
    ...
    product_tab = Product_Tab(
        product, title="Copy Engagement", tab="engagements"
    )
    return render(
        request,
...
def view_engagement(request, eid):
    ...
    tests_filter = EngagementTestFilter(
        request.GET, queryset=tests, engagement=eng
    )
    paged_tests = get_page_items(request, tests_filter.qs, default_page_num)
    # prefetch only after creating the filters to avoid
    # https://code.djangoproject.com/ticket/23771 and
    ...
    if isinstance(
        ...
        prefetched = prefetched.annotate(
            count_findings_test_mitigated=Count(
                "finding__id",
                filter=Q(finding__is_mitigated=True),
                distinct=True,
            )
        )
        prefetched = prefetched.annotate(
...
def add_tests(request, eid):
        ...
        if form.is_valid():
            ...
            if cred_form.is_valid():
                if cred_form.cleaned_data["cred_user"]:
                    # Select the credential mapping object from the selected
                    # list and only allow if the credential is associated with
                    # the product
    ...
    product_tab = Product_Tab(
        eng.product, title="Add Tests", tab="engagements"
    )
    product_tab.setEngagement(eng)
    return render(
        request,
...
# Cant use the easy decorator because of the potential for either eid/pid
# being used
def import_scan_results(request, eid=None, pid=None):
    ...
    if request.method == "POST":
        ...
        if form.is_valid() and (jform is None or jform.is_valid()):
            ...
            close_old_findings_product_scope = form.cleaned_data.get(
                "close_old_findings_product_scope", None
            )
            if close_old_findings_product_scope:
                close_old_findings = True
            # Will save in the provided environment or in the `Development` one
            # if absent
            ...
            # Allows for a test to be imported with an engagement created on
            # the fly
...
def complete_checklist(request, eid):
    ...
    product_tab = Product_Tab(
        eng.product, title="Checklist", tab="engagements"
    )
    product_tab.setEngagement(eng)
    return render(
        request,
...
def add_risk_acceptance(request, eid, fid=None):
    ...
    product_tab = Product_Tab(
        eng.product, title="Risk Acceptance", tab="engagements"
    )
    product_tab.setEngagement(eng)
...
def view_risk_acceptance(request, eid, raid):
    return view_edit_risk_acceptance(
        request, eid=eid, raid=raid, edit_mode=False
    )
...
def edit_risk_acceptance(request, eid, raid):
    return view_edit_risk_acceptance(
        request, eid=eid, raid=raid, edit_mode=True
    )
...
def view_edit_risk_acceptance(request, eid, raid, edit_mode=False):
    ...
    if request.method == "POST":
        # deleting before instantiating the form otherwise django messes up and
        # we end up with an empty path value
        ...
        if "decision" in request.POST:
            ...
            if not errors:
                logger.debug(
                    "path: %s", risk_acceptance_form.cleaned_data["path"]
                )

                risk_acceptance_form.save()

                if risk_acceptance.expiration_date != old_expiration_date:
                    # risk acceptance was changed, check if risk acceptance
                    # needs to be reinstated and findings made accepted again
        ...
        if "remove_finding" in request.POST:
            ...
            ra_helper.remove_finding_from_risk_acceptance(
                risk_acceptance, finding
            )
    ...
    accepted_findings = risk_acceptance.accepted_findings.order_by(
        "numerical_severity"
    )
    ...
    unaccepted_findings = (
        Finding.objects.filter(
            test__in=eng.test_set.all(), risk_accepted=False
        )
        .exclude(id__in=accepted_findings)
        .order_by("title")
    )
    add_fpage = get_page_items(request, unaccepted_findings, 10, "apage")
    # on this page we need to add unaccepted findings as possible findings to
    # add as accepted
    add_findings_form.fields[
        "accepted_findings"
    ].queryset = add_fpage.object_list
...
def download_risk_acceptance(request, eid, raid):
    ...
    response = StreamingHttpResponse(
        FileIterWrapper(
            open(
                settings.MEDIA_ROOT + "/" + risk_acceptance.path.name,
                mode="rb",
            )
        )
    )
...
def get_engagements(request):
    ...
    if not url:
        raise ValidationError(
            "Please use the export button when exporting engagements"
        )
    else:
    ...
    for engagement in engagements:
        ...
        if row_num > 1:
            ...
            worksheet.cell(
                row=row_num,
                column=col_num,
                value=test_counts.get(engagement.id, 0),
            )
        row_num += 1
