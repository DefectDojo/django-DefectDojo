...
class TestHuskyCIReportParser(TestCase):
    ...
    def test_parse_file_has_one_finding_one_tool(self):
        testfile = open("dojo/unittests/scans/huskyci/huskyci_report_one_finding_one_tool.json")
        parser = HuskyCIReportParser(testfile, Test())
        testfile.close()
        self.assertEqual(1, len(parser.items))

    def test_parse_file_has_many_finding_one_tool(self):
        testfile = open("dojo/unittests/scans/huskyci/huskyci_report_many_finding_one_tool.json")
        parser = HuskyCIReportParser(testfile, Test())
        testfile.close()
