...
class TestAcunetixParser(TestCase):
    ...
    def test_parse_file_with_one_finding(self):
        testfile = open(path.join(path.dirname(__file__), "scans/acunetix/one_finding.xml"))
        parser = AcunetixParser()
        findings = parser.get_findings(testfile, Test())
        self.assertEqual(1, len(findings))

    def test_parse_file_with_multiple_finding(self):
        testfile = open(path.join(path.dirname(__file__), "scans/acunetix/many_findings.xml"))
        parser = AcunetixParser()
        findings = parser.get_findings(testfile, Test())
