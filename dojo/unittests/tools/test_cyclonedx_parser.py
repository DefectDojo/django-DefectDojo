...
class TestParser(TestCase):
    ...
    def test_grype_report(self):
        with open(path.join(path.dirname(__file__), "scans/cyclonedx/grype_dd_1_14_1.xml")) as file:
    ...
    def test_spec1_report(self):
        ...
        with open(path.join(path.dirname(__file__), "scans/cyclonedx/spec1.xml")) as file:
            parser = CycloneDXParser()
            findings = list(parser.get_findings(file, Test()))
            for finding in findings:
                self.assertIn(finding.severity, Finding.SEVERITIES)
    ...
    def test_cyclonedx_bom_report(self):
        with open(path.join(path.dirname(__file__), "scans/cyclonedx/cyclonedx_bom.xml")) as file:
            parser = CycloneDXParser()
    ...
    def test_cyclonedx_jake_report(self):
        ...
        with open(path.join(path.dirname(__file__), "scans/cyclonedx/jake.xml")) as file:
            parser = CycloneDXParser()
            findings = parser.get_findings(file, Test())
            for finding in findings:
                self.assertIn(finding.severity, Finding.SEVERITIES)
