...
class TestZapParser(TestCase):
    ...
    def test_parse_some_findings_3(self):
        testfile = open("dojo/unittests/scans/zap/3_zap_sampl_0_and_different_severities.xml")
        parser = ZapParser()
        findings = parser.get_findings(testfile, Test())
        self.assertIsInstance(findings, list)
        self.assertEqual(4, len(findings))
    ...
    def test_parse_issue4697(self):
        ...
        """
        testfile = open("dojo/unittests/scans/zap/zap-results-first-scan.xml")
        parser = ZapParser()
        findings = parser.get_findings(testfile, Test())
        for finding in findings:
            for endpoint in finding.unsaved_endpoints:
    ...
    def test_parse_juicy(self):
        ...
        testfile = open("dojo/unittests/scans/zap/juicy2.xml")
        parser = ZapParser()
        findings = parser.get_findings(testfile, Test())
        for finding in findings:
            for endpoint in finding.unsaved_endpoints:
