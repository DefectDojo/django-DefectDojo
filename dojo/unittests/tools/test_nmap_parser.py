...
class TestNmapParser(TestCase):
    ...
    def test_parse_file_with_no_open_ports_has_no_findings(self):
        testfile = open(path.join(path.dirname(__file__), "scans/nmap/nmap_0port.xml"))
        parser = NmapParser()
        findings = parser.get_findings(testfile, Test())
        self.assertEqual(0, len(findings))

    def test_parse_file_with_single_open_ports_has_single_finding(self):
        testfile = open(path.join(path.dirname(__file__), "scans/nmap/nmap_1port.xml"))
        parser = NmapParser()
        findings = parser.get_findings(testfile, Test())
        self.assertEqual(1, len(findings))

    def test_parse_file_with_multiple_open_ports_has_multiple_finding(self):
        testfile = open(path.join(path.dirname(__file__), "scans/nmap/nmap_multiple_port.xml"))
        parser = NmapParser()
        findings = parser.get_findings(testfile, Test())
        self.assertEqual(13, len(findings))

    def test_parse_file_with_script_vulner(self):
        testfile = open(path.join(path.dirname(__file__), "scans/nmap/nmap_script_vulners.xml"))
        parser = NmapParser()
        findings = parser.get_findings(testfile, Test())
