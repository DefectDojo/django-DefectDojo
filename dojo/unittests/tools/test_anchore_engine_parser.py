...
class TestAnchoreEngineParser(TestCase):
    def test_anchore_engine_parser_has_no_finding(self):
        testfile = open(path.join(path.dirname(__file__), "scans/anchore/no_vuln.json"))
        parser = AnchoreEngineParser()
        findings = parser.get_findings(testfile, Test())
        self.assertEqual(0, len(findings))

    def test_anchore_engine_parser_has_one_finding(self):
        testfile = open(path.join(path.dirname(__file__), "scans/anchore/one_vuln.json"))
        parser = AnchoreEngineParser()
        findings = parser.get_findings(testfile, Test())
        testfile.close()
    ...
    def test_anchore_engine_parser_has_many_findings(self):
        testfile = open(path.join(path.dirname(__file__), "scans/anchore/many_vulns.json"))
        parser = AnchoreEngineParser()
        findings = parser.get_findings(testfile, Test())
        testfile.close()
    ...
    def test_anchore_engine_parser_has_many_findings_2_4_1(self):
        testfile = open(path.join(path.dirname(__file__), "scans/anchore/many_vulns_2.4.1.json"))
        parser = AnchoreEngineParser()
        findings = parser.get_findings(testfile, Test())
        testfile.close()
