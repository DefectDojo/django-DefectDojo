...
class TesthadolintParser(TestCase):
    ...
    def test_parse_file_with_one_dockerfile(self):
        testfile = open(path.join(path.dirname(__file__), "scans/hadolint/one_dockerfile.json"))
    ...
    def test_parse_file_with_many_dockerfile(self):
        testfile = open(path.join(path.dirname(__file__), "scans/hadolint/many_dockerfile.json"))
        parser = HadolintParser()
        findings = parser.get_findings(testfile, Test())
        testfile.close()
