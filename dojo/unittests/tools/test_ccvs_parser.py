...
class TestCCVSParser(TestCase):
    def test_ccvs_parser_has_no_finding(self):
        testfile = open(path.join(path.dirname(__file__), "scans/ccvs/no_vuln.json"))
        parser = CCVSParser()
        findings = parser.get_findings(testfile, Test())
        self.assertEqual(0, len(findings))

    def test_ccvs_parser_has_one_finding(self):
        testfile = open(path.join(path.dirname(__file__), "scans/ccvs/one_vuln_one_vendor.json"))
        parser = CCVSParser()
        findings = parser.get_findings(testfile, Test())
        testfile.close()
    ...
    def test_ccvs_parser_has_many_findings(self):
        testfile = open(path.join(path.dirname(__file__), "scans/ccvs/many_vulns.json"))
        parser = CCVSParser()
        findings = parser.get_findings(testfile, Test())
        testfile.close()
