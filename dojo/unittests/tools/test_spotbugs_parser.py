...
class TestSpotbugsParser(TestCase):
    ...
    def test_no_findings(self):
        ...
        findings = parser.get_findings(path.join(path.dirname(__file__), "scans/spotbugs/no_finding.xml"), Test())
        self.assertEqual(0, len(findings))

    def test_parse_many_finding(self):
        parser = SpotbugsParser()
        findings = parser.get_findings(path.join(path.dirname(__file__), "scans/spotbugs/many_findings.xml"), Test())
