...
class TestSolarAppscreenerParser(TestCase):
    ...
    def test_solar_appscreener_parser_with_no_vuln_has_no_findings(self):
        testfile = open(
            "dojo/unittests/scans/solar_appscreener/solar_appscreener_zero_vul.csv")
    ...
    def test_solar_appscreener_parser_with_one_criticle_vuln_has_one_findings(self):
        testfile = open(
            "dojo/unittests/scans/solar_appscreener/solar_appscreener_one_vul.csv")
        parser = SolarAppscreenerParser()
        findings = parser.get_findings(testfile, Test())
        testfile.close()
    ...
    def test_solar_appscreener_parser_with_many_vuln_has_many_findings(self):
        testfile = open(
            "dojo/unittests/scans/solar_appscreener/solar_appscreener_many_vul.csv")
        parser = SolarAppscreenerParser()
        findings = parser.get_findings(testfile, Test())
        testfile.close()
        for finding in findings:
