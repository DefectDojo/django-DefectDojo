...
class TestSonarQubeParser(TestCase):
    ...
    def test_parse_file_with_table_in_table(self):
        my_file_handle = open("dojo/unittests/scans/sonarqube/sonar-table-in-table.html")
        product = Product()
        engagement = Engagement()
        test = Test()
        engagement.product = product
        ...
        self.assertMultiLineEqual(
            ...
            item.references)
        self.assertEqual(str, type(item.file_path))
        self.assertEqual("tomcat_180410:java/org/apache/catalina/util/URLEncoder.java", item.file_path)
        self.assertEqual(str, type(item.line))
    ...
    def test_parse_file_with_rule_undefined(self):
        my_file_handle = open("dojo/unittests/scans/sonarqube/sonar-rule-undefined.html")
        product = Product()
        engagement = Engagement()
        test = Test()
        engagement.product = product
