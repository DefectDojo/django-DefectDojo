...
class TestRiskAcceptanceApi(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        Finding.objects.bulk_create(
            map(lambda i: create_finding(cls.test_a, cls.user, 'CVE-1999-{}'.format(i)), range(500, 1500, 3)))
        Finding.objects.bulk_create(
            map(lambda i: create_finding(cls.test_b, cls.user, 'CVE-1999-{}'.format(i)), range(501, 1500, 3)))
        Finding.objects.bulk_create(
            map(lambda i: create_finding(cls.test_c, cls.user, 'CVE-1999-{}'.format(i)), range(502, 1500, 3)))
    ...
    def test_test_accept_risks(self):
        accepted_risks = [{'cve': 'CVE-1999-{}'.format(i), 'justification': 'Demonstration purposes',
                           'accepted_by': 'King of the Internet'} for i in range(1000, 1500)]
        result = self.client.post(reverse('test-accept-risks', kwargs={'pk': self.test_a.id}), data=accepted_risks,
                                  format='json')
        self.assertEquals(len(result.json()), 167)
        self.assertEquals(self.test_a.unaccepted_open_findings.count(), 167)
        self.assertEquals(self.test_b.unaccepted_open_findings.count(), 333)
        self.assertEquals(self.test_c.unaccepted_open_findings.count(), 333)
    ...
    def test_engagement_accept_risks(self):
        ...
        self.assertEquals(len(result.json()), 500)
        self.assertEquals(self.engagement.unaccepted_open_findings.count(), 500)

    def test_finding_accept_risks(self):
        accepted_risks = [{'cve': 'CVE-1999-{}'.format(i), 'justification': 'Demonstration purposes',
                           'accepted_by': 'King of the Internet'} for i in range(600, 1400)]
        result = self.client.post(reverse('finding-accept-risks'), data=accepted_risks, format='json')
        self.assertEquals(len(result.json()), 800)
        self.assertEquals(Finding.unaccepted_open_findings().count(), 200)
