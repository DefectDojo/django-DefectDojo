...
def create_notification(event=None, **kwargs):
    ...
    if "recipients" in kwargs:
        ...
        logger.debug(
            "creating notifications for recipients: %s", kwargs["recipients"]
        )
        for recipient_notifications in Notifications.objects.filter(
            ...
            logger.debug(
                "Sent notification to %s", recipient_notifications.user
            )
        ...
        try:
            system_notifications = Notifications.objects.get(
                user=None, template=False
            )
        except Exception:
            system_notifications = Notifications()

        # System notifications are sent one with user=None, which will trigger
        # email to configured system email, to global slack channel, etc.
        process_notifications(event, system_notifications, **kwargs)

        # All admins will also receive system notifications, \
        # but as part of the person global notifications section below
        ...
        if not ("no_users" in kwargs and kwargs["no_users"] is True):
            ...
            for user in users:
                ...
                # send notifications to user after merging possible multiple notifications records \
                # (i.e. personal global + personal product)
...
def create_notification_message(
    event, user, notification_type, *args, **kwargs
):
    template = (
        f"notifications/{notification_type}/{event.replace('/', '')}.tpl"
    )
    kwargs.update({"user": user})
    ...
    try:
        ...
        if not notification_message:
            ...
            notification_message = render_to_string(
                f"notifications/{notification_type}/other.tpl", kwargs
            )
...
def get_slack_user_id(user_email):
    ...
    if user:
        if "error" in user:
            ...
            logger.error(user)
            raise RuntimeError(
                "Error getting user list from Slack: " + res.text
            )
        else:
...
def notify_scan_added(
    ...
    new_findings = sorted(
        list(new_findings), key=lambda x: x.numerical_severity
    )
