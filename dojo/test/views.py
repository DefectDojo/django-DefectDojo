...
def view_test(request, tid):
    ...
    # tests_prefetched = test_prefetched.prefetch_related(
    #     Prefetch(
    #         'test_import_set',
    #         queryset=Test_Import.objects.filter(~Q(findings_affected=None))))
    # tests_prefetched = test_prefetched.prefetch_related('test_import_set')
    # test_prefetched = test_prefetched.prefetch_related(
    #     'test_import_set__test_import_finding_action_set')
    ...
    paged_test_imports.object_list = (
        paged_test_imports.object_list.prefetch_related(
    ...
    show_re_upload = any(
        test.test_type.name in code for code in get_choices_sorted()
    )
...
# if isinstance(test_imports, QuerySet):
# old code can arrive here with prods being a list because the query was already executed
#         #could we make this dynamic, i.e for action_type in IMPORT_ACTIONS: prefetch
# prefetched_test_imports = prefetched_test_imports.annotate(
#     created_findings_count=Count(
#         'findings', filter=Q(
#             test_import_finding_action__action=IMPORT_CREATED_FINDING)))
# prefetched_test_imports = prefetched_test_imports.annotate(
#     closed_findings_count=Count(
#         'findings', filter=Q(test_import_finding_action__action=IMPORT_CLOSED_FINDING)))
# prefetched_test_imports = prefetched_test_imports.annotate(
#     reactivated_findings_count=Count('findings', filter=Q(
# test_import_finding_action__action=IMPORT_REACTIVATED_FINDING)))
# prefetched_test_imports = prefetched_test_imports.annotate(
#     updated_findings_count=Count(
#         'findings', filter=Q(test_import_finding_action__action=IMPORT_UNTOUCHED_FINDING)))
        ...
        if "id" in request.POST and str(test.id) == request.POST["id"]:
            ...
            if form.is_valid():
                ...
                create_notification(
                    ...
                    product=product,
                    description=_(
                        'The test "%(title)s" was deleted by %(user)s'
                    )
...
def copy_test(request, tid):
    ...
    product = test.engagement.product
    engagement_list = get_authorized_engagements(
...
def add_findings(request, tid):
    ...
    if request.method == "POST":
        ...
        if (
            form["active"].value() is False or form["false_p"].value()
        ...
        if form.is_valid() and (jform is None or jform.is_valid()):
            ...
            if (
                "request" in form.cleaned_data
                or "response" in form.cleaned_data
            ):
...
def re_import_scan_results(request, tid):
    ...
    if get_system_setting("enable_jira") and jira_project:
        jform = JIRAImportScanForm(
            push_all=push_all_jira_issues, prefix="jiraform"
        )
    ...
    if request.method == "POST":
        ...
        if form.is_valid() and (jform is None or jform.is_valid()):
            ...
            close_old_findings = form.cleaned_data.get(
            ...
            if scan and is_scan_file_too_large(scan):
                messages.add_message(
                    ...
                    messages.ERROR,
                    _(
                        "Report file is too large. Maximum supported size is %(size)d MB"
                    )
