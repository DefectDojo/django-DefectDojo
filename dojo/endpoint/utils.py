...
def endpoint_filter(**kwargs):
    ...
    if kwargs.get("port"):
        ...
        if (kwargs.get("protocol")) and (
            kwargs["protocol"].lower() in SCHEME_PORT_MAP
        ):
            qs = qs.filter(
                Q(port__isnull=True)
                | Q(port__exact=SCHEME_PORT_MAP[kwargs["protocol"].lower()])
...
def clean_hosts_run(apps, change):
    def err_log(message, html_log, endpoint_html_log, endpoint):
        error_suffix = (
            "It is not possible to migrate it. Delete or edit this endpoint."
        )
        html_log.append({**endpoint_html_log, **{"message": message}})
        logger.error(
            "Endpoint (id={}) {}. {}".format(
                endpoint.pk, message, error_suffix
            )
        )
    ...
    for endpoint in endpoints:
        try:
            ...
            for ve in ves:
                errors.append(
                    ValidationError(
                        "Invalid endpoint {}: {}".format(endpoint, ve)
                    )
                )
...
def endpoint_meta_import(
    ...
    for row in reader:
        ...
        for endpoint in endpoints:
            ...
            for item in meta:
                if create_meta:
                    # check if meta exists first. Don't want to make duplicate
                    # endpoints
                ...
                # if tags are not supposed to be added, this value remain
                # unchanged
...
def remove_broken_endpoint_statuses(apps):
    ...
    broken_eps = Endpoint_Status.objects.filter(
        Q(endpoint=None) | Q(finding=None)
    )
    if broken_eps.count() == 0:
        logger.info("There is no broken endpoint_status")
    else:
        logger.warning(
            "We identified %s broken endpoint_statuses", broken_eps.count()
        )
