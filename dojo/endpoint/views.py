...
def process_endpoints_view(request, host_view=False, vulnerable=False):
    ...
    add_breadcrumb(
        title=view_name, top_level=not len(request.GET), request=request
    )
...
def process_endpoint_view(request, eid, host_view=False):
    ...
    # closed_findings is needed as a parameter for get_periods_counts, but
    # they are not relevant in the endpoint view
...
def edit_endpoint(request, eid):
    ...
    if request.method == "POST":
        ...
        add_breadcrumb(
            parent=endpoint, title="Edit", top_level=False, request=request
        )
...
def delete_endpoint(request, eid):
    ...
    product_tab = Product_Tab(
        endpoint.product, "Delete Endpoint", tab="endpoints"
    )
...
def add_meta_data(request, eid):
    ...
    if request.method == "POST":
        form = DojoMetaDataForm(
            request.POST, instance=DojoMeta(endpoint=endpoint)
        )
        if form.is_valid():
            form.save()
            messages.add_message(
    ...
    product_tab = Product_Tab(
        endpoint.product, "Add Metadata", tab="endpoints"
    )
    return render(
        request,
...
def edit_meta_data(request, eid):
    ...
    product_tab = Product_Tab(
        endpoint.product, "Edit Metadata", tab="endpoints"
    )
    return render(
        request,
...
# bulk mitigate and delete are combined, so we can't have the nice
# user_is_authorized decorator
def endpoint_bulk_update_all(request, pid=None):
    if request.method == "POST":
        endpoints_to_update = request.POST.getlist("endpoints_to_update")
        endpoints = Endpoint.objects.filter(
            id__in=endpoints_to_update
        ).order_by("endpoint_meta__product__id")
        ...
        if request.POST.get("delete_bulk_endpoints") and endpoints_to_update:
            ...
            product_calc = list(
                Product.objects.filter(
                    endpoint__id__in=endpoints_to_update
                ).distinct()
            )
            endpoints.delete()
...
def import_endpoint_meta(request, pid):
    ...
    add_breadcrumb(
        title="Endpoint Meta Importer", top_level=False, request=request
    )
    product_tab = Product_Tab(
        product, title="Endpoint Meta Importer", tab="endpoints"
    )
    return render(
        request,
