...
def configure_announcement(request):
        ...
        if form.is_valid() and created:
            ...
            if created:
                UserAnnouncement.objects.bulk_create(
                    [
                        UserAnnouncement(user=user_id, announcement=announcement)
    ...
    add_breadcrumb(
        title=gettext("Announcement Configuration"), top_level=True, request=request
    )
    return render(request, "dojo/announcement.html", {"form": form, "remove": remove})
