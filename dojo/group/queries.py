...
def get_authorized_group_members(permission):
    ...
    return Dojo_Group_Member.objects.filter(group__in=groups).select_related(
        "role"
    )
...
def get_product_type_groups_for_group(group):
    return Product_Type_Group.objects.filter(group=group).select_related(
        "role"
    )
