...
def pre_save_finding_status_change(
    sender, instance, changed_fields=None, **kwargs
):
    ...
    logger.debug(
        "%i: changed status fields pre_save: %s",
        instance.id or 0,
        changed_fields,
    )
...
def update_finding_status(new_state_finding, user, changed_fields=None):
    ...
    if is_new_finding or "is_mitigated" in changed_fields:
        ...
        if new_state_finding.is_mitigated:
            # when mitigating a finding, the meta fields can only be editted if
            # allowed
            logger.debug(
            ...
            if can_edit_mitigated_data(user):
                ...
                # it will cause all kinds of issues I believe with new findings
                # etc
                new_state_finding.mitigated = (
                    new_state_finding.mitigated or now
                )
                new_state_finding.mitigated_by = (
                    new_state_finding.mitigated_by or user
                )
...
def create_finding_group(finds, finding_group_name):
    ...
    finding_group.save()
    available_findings = [
        find for find in finds if not find.finding_group_set.all()
    ]
    finding_group.findings.set(available_findings)

    # if user provided a name, we use that, else:
    # if we have components, we may set a nice name but catch 'name already
    # exist' exceptions
    try:
...
def add_to_finding_group(finding_group, finds):
    ...
    if (
        ...
        and jira_helper.get_jira_instance(finding_group).finding_jira_sync
    ):
...
def remove_from_finding_group(finds):
    ...
    for group in affected_groups:
        if (
            group.has_jira_issue
            and jira_helper.get_jira_instance(group).finding_jira_sync
        ):
            logger.debug(
...
def update_finding_group(finding, finding_group):
    ...
    if finding_group is not None:
        if finding_group != finding.finding_group:
            ...
            logger.debug(
                "adding finding %d to finding_group %s",
                finding.id,
                finding_group,
            )
...
def get_group_by_group_name(finding, finding_group_by_option):
    ...
    if finding_group_by_option == "component_name":
        ...
        raise ValueError(
            "Invalid group_by option %s" % finding_group_by_option
        )
...
def group_findings_by(finds, finding_group_by_option):
    ...
    for find in finds:
        ...
        finding_group = Finding_Group.objects.filter(
            test=find.test, name=group_name
        ).first()
        if not finding_group:
            finding_group, added, skipped = create_finding_group(
                [find], group_name
            )
            groups_created += 1
            grouped += added
...
def add_findings_to_auto_group(
    name,
    findings,
    group_by,
    ...
    **kwargs
    ...
    if name is not None and findings is not None and len(findings) > 0:
        ...
        if create_finding_groups_for_all_findings or len(findings) > 1:
            # Only create a finding group if we have more than one finding for
            # a given finding group, unless configured otherwise
            ...
            try:
                ...
                # See if we have old findings in the same test that were created without a finding group
                # that match this new finding - then we can create a finding
                # group
                old_findings = Finding.objects.filter(test=test)
                created = False
                for f in old_findings:
                    ...
                    if f_group_name == name and f not in findings:
                        (
                            finding_group,
                            created,
                        ) = Finding_Group.objects.get_or_create(
                            test=test, creator=creator, name=name
                        )
                        finding_group.findings.add(f)
                if created:
...
def post_process_finding_save(
    ...
    **kwargs
    ...
    # STEP 1 run all status changing tasks sequentially to avoid race
    # conditions
    if dedupe_option:
    ...
    # STEP 2 run all non-status changing tasks as celery tasks in the
    # background
    ...
    if push_to_jira:
        logger.debug(
            "pushing finding %s to jira from finding.save()", finding.pk
        )
        import dojo.jira_link.helper as jira_helper
        ...
        # but what about the push_all boolean? Let's see how this works for now
...
def reconfigure_duplicate_cluster(original, cluster_outside):
    ...
    if (
        original is None
        or cluster_outside is None
        or len(cluster_outside) == 0
    ):
        return
    ...
    if settings.DUPLICATE_CLUSTER_CASCADE_DELETE:
        ...
        logger.debug(
            "reconfigure_duplicate_cluster: cluster_outside: %s",
            cluster_outside,
        )
...
def prepare_duplicates_for_delete(test=None, engagement=None):
    ...
    # remove the link to the original from the duplicates inside the cluster
    # so they can be safely deleted by the django framework
    total = len(originals)
    i = 0
    ...
    for original in originals:
        ...
        if engagement:
            cluster_outside = cluster_outside.exclude(
        ...
        logger.debug(
            "done preparing duplicate cluster for deletion of original: %d",
            original.id,
        )
...
def fix_loop_duplicates():
    ...
    if loop_count > 0:
        deduplicationLogger.info(
            "Identified %d Findings with Loops" % len(candidates)
        )
...
def removeLoop(finding_id, counter):
    ...
    if finding_id == real_original.id:
        ...
        # duplicate remains True, will be set to False in fix_loop_duplicates
        # (and logged as New Original?).
        super(Finding, finding).save()
        return

    # Only modify the findings if the original ID is lower to get the oldest
    # finding as original
    ...
    for f in finding.original_finding.all():
        # for all duplicates set the original as their original, get rid of
        # self in between
...
def save_vulnerability_ids_template(finding_template, vulnerability_ids):
    ...
    # Remove old vulnerability ids
    Vulnerability_Id_Template.objects.filter(
