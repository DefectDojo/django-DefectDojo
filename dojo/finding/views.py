...
def get_filtered_findings(
    ...
    if filter_name == "Open":
        ...
        findings = findings.filter(
            finding_helper.FALSE_POSITIVE_FINDINGS_QUERY
        )
...
def verified_findings(request, pid=None, eid=None, view=None):
    return findings(
        request, pid=pid, eid=eid, view=view, filter_name="Verified"
    )


def out_of_scope_findings(request, pid=None, eid=None, view=None):
    return findings(
        request, pid=pid, eid=eid, view=view, filter_name="Out of Scope"
    )


def false_positive_findings(request, pid=None, eid=None, view=None):
    return findings(
        request, pid=pid, eid=eid, view=view, filter_name="False Positive"
    )


def inactive_findings(request, pid=None, eid=None, view=None):
    return findings(
        request, pid=pid, eid=eid, view=view, filter_name="Inactive"
    )


def accepted_findings(request, pid=None, eid=None, view=None):
    return findings(
        request, pid=pid, eid=eid, view=view, filter_name="Accepted"
    )
...
def findings(
    ...
    # trick to prefetch after paging to avoid huge join generated by select
    # count(*) from Paginator
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if len(missing_note_types) <= 1:
                ...
                finding.out_of_scope = form.cleaned_data.get(
                    "out_of_scope", False
                )
                ...
                # only push to JIRA if there is an issue, to prevent a new one
                # from being created
                if (
                    jira_helper.is_push_all_issues(finding)
                    and finding.has_jira_issue
                ):
...
def defect_finding_review(request, fid):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # TODO: JIRA: Code below should move to jira_helper. But I have no
            # idea what it is doin so don't want move/break it
    ...
    if request.method == "POST":
        ...
        if finding.active:
            if (
                ...
                note_type_activation = Note_Type.objects.filter(
                    is_active=True
                ).count()
                closing_disabled = 0
                if note_type_activation:
                    closing_disabled = len(
                        get_missing_mandatory_notetypes(finding)
                    )
        ...
        if form.is_valid() and (jform is None or jform.is_valid()):
            ...
            if (
                "risk_accepted" in form.cleaned_data
                and form["risk_accepted"].value()
            ):
                if (
                    new_finding.test.engagement.product.enable_simple_risk_acceptance
                ):
                    ra_helper.simple_risk_accept(
                        new_finding, perform_save=False
                    )
            else:
            ...
            endpoint_status_list = Endpoint_Status.objects.filter(
                finding=new_finding
            )
            ...
            # If active is not checked and CAN_EDIT_MIIGATED_DATA, mitigate the
            # finding and the associated endpoints status
            if finding_helper.can_edit_mitigated_data(request.user):
                if (
                    ...
                    for status in endpoint_status:
                        status.mitigated_by = (
                            form.cleaned_data.get("mitigated_by")
                            or request.user
                        )
                        status.mitigated_time = (
                            form.cleaned_data.get("mitigated") or now
            ...
            if (
                ...
                burp_rr = BurpRawRequestResponse.objects.filter(
                    finding=finding
                ).first()
                if burp_rr:
            ...
            if jform and jform.is_valid():
                ...
                logger.debug(
                    "has_jira_group_issue: %s",
                    new_finding.has_jira_group_issue,
                )
                ...
                if not new_finding.has_jira_group_issue:
                    if new_finding.has_jira_issue:
                        ...
                        if not new_jira_issue_key:
                            ...
                            new_jira_issue_key
                            != new_finding.jira_issue.jira_key
                        ):
                            jira_helper.finding_unlink_jira(
                                request, new_finding
                            )
            ...
            if "githubform-push_to_github" in request.POST:
                ...
                if gform.is_valid():
                    if GITHUB_Issue.objects.filter(
                        finding=new_finding
                    ).exists():
                        update_external_issue(
                            new_finding, old_status, "github"
                        )
                    else:
            ...
            # do not relaunch deduplication, otherwise, it's never taken into
            # account
            if old_finding.duplicate and not new_finding.duplicate:
                new_finding.duplicate_finding = None
                new_finding.save(
                    push_to_jira=push_to_jira, dedupe_option=False
                )
            else:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            for suser in form.cleaned_data["reviewers"]:
                ...
                reviewers_short.append(
                    Dojo_User.objects.get(id=suser).username
                )
...
def find_template_to_apply(request, fid):
    ...
    # just query all templates as this weird ordering above otherwise breaks
    # Django ORM
    title_words = get_words_for_field(Finding_Template, "title")
    product_tab = Product_Tab(
        test.engagement.product,
...
def promote_to_finding(request, fid):
    ...
    if request.method == "POST":
        form = PromoteFindingForm(
            request.POST, product=test.engagement.product
        )
        if use_jira:
            jform = JIRAFindingForm(
                request.POST,
...
def export_templates_to_json(request):
    leads_as_json = serializers.serialize(
        "json", Finding_Template.objects.all()
    )
...
def apply_cwe_mitigation(apply_to_findings, template, update=True):
    ...
    if (
        apply_to_findings
        and template.template_match
        and template.cwe is not None
    ):
        # Update active, verified findings with the CWE template
        # If CWE only match only update issues where there isn't a CWE + Title
        # match
        if template.template_match_title:
            ...
            if update:
                # MySQL won't allow an 'update in statement' so loop will have
                # to do
...
def merge_finding_product(request, pid):
    ...
    if (
        ...
        if request.method == "POST":
            form = MergeFindings(
                request.POST, finding=finding, findings=findings
            )
            if form.is_valid():
                ...
                if finding_to_merge_into not in findings_to_merge:
                    for finding in findings_to_merge.exclude(
                        ...
                        # if checked re-assign the burp requests to the merged
                        # finding
                        ...
                        if form.cleaned_data["finding_action"] == "inactive":
                            ...
                            # If the merged finding should be tagged as
                            # merged-into
                    ...
                    if finding_to_merge_into.line is None:
                        finding_to_merge_into.line

                    if finding_to_merge_into.file_path is None:
                        finding_to_merge_into.file_path
                    ...
                    return HttpResponseRedirect(
                        reverse(
                            "edit_finding", args=(finding_to_merge_into.id,)
                        )
                    )
                else:
                    messages.add_message(
...
# bulk update and delete are combined, so we can't have the nice
        ...
        if request.POST.get("delete_bulk_findings"):
            ...
            if form.is_valid() and finding_to_update:
                ...
                if (
                    ...
                    for find in finds:
                        ...
                        if form.cleaned_data["status"]:
                            ...
                            find.out_of_scope = form.cleaned_data[
                                "out_of_scope"
                            ]
                            find.is_mitigated = form.cleaned_data[
                                "is_mitigated"
                            ]
                ...
                if form.cleaned_data["finding_group_create"]:
                    ...
                    finding_group_name = form.cleaned_data[
                        "finding_group_create_name"
                    ]
                    logger.debug(
                        "finding_group_create_name: %s", finding_group_name
                    )
                    (
                        finding_group,
                        added,
                ...
                if form.cleaned_data["finding_group_by"]:
                    ...
                    logger.debug(
                        "finding_group_by_option: %s", finding_group_by_option
                    )
                    ...
                    (
                        ...
                        skipped,
                        groups_created,
                    ) = finding_helper.group_findings_by(
                        finds, finding_group_by_option
                    )
                ...
                if form.cleaned_data["push_to_github"]:
                    ...
                    for finding in finds:
                        logger.debug(
                            "will push to GitHub finding: " + str(finding)
                        )
                        old_status = finding.status()
                        if form.cleaned_data["push_to_github"]:
                            if GITHUB_Issue.objects.filter(
                ...
                if success_count > 0:
                    add_success_message_to_response(
                        "%i finding groups pushed to JIRA successfully"
                        % success_count
                    )
                ...
                for finding in finds:
                    ...
                    if not groups_pushed_to_jira and (
                        ...
                        if (
                            finding.has_jira_group_issue
                            and not finding.has_jira_issue
                        ):
                ...
                if success_count > 0:
                    add_success_message_to_response(
                        "%i findings pushed to JIRA successfully"
                        % success_count
                    )
...
def set_finding_as_original_internal(user, finding_id, new_original_id):
    ...
    if finding.duplicate or finding.original_finding.all():
        ...
        logger.debug(
            "setting new original for old root %i to %i",
            finding.id,
            new_original.id,
        )
        ...
        logger.debug(
            "marking %i as duplicate of %i", finding.id, new_original.id
        )
        finding.duplicate = True
        finding.active = False
        finding.duplicate_finding = new_original
...
def unlink_jira(request, fid):
    finding = get_object_or_404(Finding, id=fid)
    logger.info(
        "trying to unlink a linked jira issue from %d:%s",
        finding.id,
        finding.title,
    )
    if finding.has_jira_issue:
    ...
    if similar_finding.test.engagement != finding.test.engagement and (
        ...
        if similar_finding.duplicate_finding:
            ...
            if finding.duplicate or finding.original_finding.all():
                ...
                # similar_finding is not an original/root of a cluster as per
                # earlier if clause
                actions.append(
                    {
                        "action": "mark_finding_duplicate",
