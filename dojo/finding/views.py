...
from dojo.filters import (
    OpenFindingFilter,
    OpenFindingSuperFilter,
    AcceptedFindingSuperFilter,
    ClosedFindingSuperFilter,
...
def open_findings_filter(request, queryset, user, pid):
    if user.is_staff:
        return OpenFindingSuperFilter(
            request.GET, queryset=queryset, user=user, pid=pid
        )
    else:
...
def open_findings(request, pid=None, eid=None, view=None):
    return findings(
        ...
        view=view,
        filter_name="Open",
        query_filter=OPEN_FINDINGS_QUERY,
    )
...
def verified_findings(request, pid=None, eid=None, view=None):
    return findings(
        ...
        query_filter=VERIFIED_FINDINGS_QUERY,
    )
...
def out_of_scope_findings(request, pid=None, eid=None, view=None):
    return findings(
        ...
        query_filter=OUT_OF_SCOPE_FINDINGS_QUERY,
    )
...
def false_positive_findings(request, pid=None, eid=None, view=None):
    return findings(
        ...
        query_filter=FALSE_POSITIVE_FINDINGS_QUERY,
    )
...
def inactive_findings(request, pid=None, eid=None, view=None):
    return findings(
        ...
        query_filter=INACTIVE_FINDINGS_QUERY,
    )
...
def accepted_findings(request, pid=None, eid=None, view=None):
    return findings(
        ...
        query_filter=ACCEPTED_FINDINGS_QUERY,
...
def closed_findings(request, pid=None, eid=None, view=None):
    return findings(
        ...
        view=view,
        filter_name="Closed",
        query_filter=CLOSED_FINDINGS_QUERY,
        order_by=("-mitigated"),
...
def findings(
    ...
    if pid:
        ...
        findings = Finding.objects.filter(test__engagement=eid).order_by(
            "numerical_severity"
        )

        show_product_column = False
        product_tab = Product_Tab(
            engagement.product_id, title=engagement.name, tab="engagements"
        )
    ...
    paged_findings = get_page_items(
        request, prefetch_for_findings(findings_filter.qs), 25
    )
...
def view_finding(request, fid):
    ...
    similar_findings_filter = SimilarFindingFilter(
        request.GET, queryset=Finding.objects.all(), user=request.user, finding=finding
    )
    logger.debug("similar query: %s", similar_findings_filter.qs.query)
    similar_findings = prefetch_for_similar_findings(
        similar_findings_filter.qs[: settings.SIMILAR_FINDINGS_MAX_RESULTS]
    )
    ...
    product_tab = Product_Tab(
        finding.test.engagement.product.id, title="View Finding", tab="findings"
    )
...
def close_finding(request, fid):
    ...
    product_tab = Product_Tab(
        finding.test.engagement.product.id, title="Close", tab="findings"
    )
...
def defect_finding_review(request, fid):
            ...
            if jira and finding.has_jira_issue:
                ...
                if defect_choice == "Close Finding":
                    ...
                    if resolution_id is None:
                        ...
                        jira_helper.jira_change_resolution_id(
                            jira, issue, resolution_id
                        )
                        new_note.entry = (
    ...
    product_tab = Product_Tab(
        finding.test.engagement.product.id, title="Jira Status Review", tab="findings"
    )
...
def edit_finding(request, fid):
    ...
    if request.method == "POST":
        form = FindingForm(
            request.POST, instance=finding, template=False, req_resp=None
        )
        ...
        if form.is_valid() and (jform is None or jform.is_valid()):
            ...
            finding_helper.update_finding_status(
                new_finding, request.user, old_state_finding=old_finding
            )
            ...
            for endpoint in form.cleaned_data["endpoints"]:
                eps, created = Endpoint_Status.objects.get_or_create(
                    finding=new_finding, endpoint=endpoint
                )
    ...
    product_tab = Product_Tab(
        finding.test.engagement.product.id, title="Edit Finding", tab="findings"
    )
...
def request_finding_review(request, fid):
    ...
    product_tab = Product_Tab(
        finding.test.engagement.product.id, title="Review Finding", tab="findings"
    )
...
def clear_finding_review(request, fid):
    ...
    product_tab = Product_Tab(
        finding.test.engagement.product.id, title="Clear Finding Review", tab="findings"
    )
...
def find_template_to_apply(request, fid):
    ...
    product_tab = Product_Tab(
        test.engagement.product.id, title="Apply Template to Finding", tab="findings"
    )
    return render(
        request,
        "dojo/templates.html",
...
def promote_to_finding(request, fid):
    ...
    product_tab = Product_Tab(
        finding.test.engagement.product.id, title="Promote Finding", tab="findings"
    )
...
def manage_images(request, fid):
    ...
    if request.method == "POST":
        ...
        if images_formset.is_valid():
            ...
            for file in files:
                with_media_root = os.path.join(
                    settings.MEDIA_ROOT, "finding_images", file
                )
...
def merge_finding_product(request, pid):
    ...
    product_tab = Product_Tab(
        finding.test.engagement.product.id, title="Merge Findings", tab="findings"
    )
    custom_breadcrumb = {
    ...
    if request.method == "POST":
        ...
        if request.POST.get("delete_bulk_findings") and finding_to_update:
            ...
            if form.is_valid() and finding_to_update:
                finding_to_update = request.POST.getlist("finding_to_update")
                finds = Finding.objects.filter(id__in=finding_to_update).order_by(
                ...
                for finding in finds:
                    ...
                    if jira_helper.is_push_all_issues(finding) or form.cleaned_data.get(
                        "push_to_jira"
                    ):
                        if not jira_helper.get_jira_project(finding):
                            jira_helper.log_jira_alert(
                                "Finding cannot be pushed to jira as there is no jira project configuration for this product.",
                                finding,
                            )
                        else:
                            logger.debug(
...
def set_finding_as_original_internal(user, finding_id, new_original_id):
    ...
    if new_original.test.engagement != new_original.test.engagement:
        if (
            new_original.test.engagement.deduplication_on_engagement
...
def get_similar_findings(request, finding):
    ...
    identical = (
        ...
        .filter(hash_code=finding.hash_code)
    ...
    temp = Finding.objects.all().filter(
        id__in=[49046, 51314, 59225, 59227, 59229, 59223]
    )
