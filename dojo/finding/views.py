...
def false_positive_findings(request, pid=None, eid=None, view=None):
    ...
    if pid:
        if view == "All":
            ...
            findings = Finding.objects.filter(test__engagement__product__id=pid, active=False, false_p=True).order_by('numerical_severity')
    elif eid:
        ...
        if view == "All":
            ...
            findings = Finding.objects.filter(test__engagement=eid, active=False, false_p=True).order_by('numerical_severity')
    else:
        if view == "All":
            filter_name = "All"
            findings = Finding.objects.all().order_by('numerical_severity')
        else:
            findings = Finding.objects.filter(active=False, false_p=True).order_by('numerical_severity')
