...
FINDING_FILTER_SPEC = register_filter_spec("finding", FilterSpec(
    ...
    filters={
        ...
        "asset": filter_field("test__engagement__product", "exact", "number"),
        "asset__in": filter_field("test__engagement__product", "in", "number"),
        "organization": filter_field("test__engagement__product__prod_type", "exact", "number"),
...
def build_findings_router(
    *,
    ...
    def list_findings(request: HttpRequest):
        ...
        # ?fields= may opt up into the detail field set (§4.7 Part A); the LIST queryset defers the
        # heavy detail columns that were not requested (Part B).
