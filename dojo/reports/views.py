...
def report_builder(request):
    ...
    endpoints = EndpointFilter(
        request.GET, queryset=endpoints, user=request.user
    )
...
def report_findings(request):
    ...
    if "test__engagement__product__prod_type" in request.GET:
        ...
        if len(p) == 1:
            pass
            # TODO: unsed variable "product_type"
...
def product_endpoint_report(request, pid):
    # user = Dojo_User.objects.get(id=request.user.id) # unsed variable user
    ...
    product_tab = Product_Tab(
        product, "Product Endpoint Report", tab="endpoints"
    )
    return render(
        request,
...
def generate_report(request, obj, host_view=False):
    ...
    if type(obj).__name__ == "Product_Type":
        ...
        user_has_permission_or_403(
            request.user, obj, Permissions.Engagement_View
        )
    elif type(obj).__name__ == "Test":
        user_has_permission_or_403(request.user, obj, Permissions.Test_View)
    elif type(obj).__name__ == "Endpoint":
        user_has_permission_or_403(
            request.user, obj, Permissions.Endpoint_View
        )
    elif (
        ...
        if report_format == "AsciiDoc":
            return render(
                ...
                "dojo/asciidoc_report.html",
                {
                    ...
                    "test": test,
                    "endpoint": endpoint,
                    "findings": findings.qs.distinct().order_by(
                        "numerical_severity"
                    ),
                    "include_finding_notes": include_finding_notes,
                    "include_finding_images": include_finding_images,
    ...
    if engagement:
        ...
        product_tab = Product_Tab(
            product, title="Product Report", tab="findings"
        )
    elif endpoints:
        if host_view:
            product_tab = Product_Tab(
...
def get_findings(request):
    ...
    if obj_id:
        if "product" in obj_name:
            ...
            user_has_permission_or_403(
                request.user, obj, Permissions.Product_View
            )
            ...
            user_has_permission_or_403(
                request.user, obj, Permissions.Test_View
            )
    ...
    for finding in findings:
        if row_num == 1:
            ...
            for key in dir(finding):
                if (
                    ...
                    cell = worksheet.cell(
                        row=row_num, column=col_num, value=key
                    )
                    cell.font = font_bold
                    col_num += 1
            cell = worksheet.cell(
                row=row_num, column=col_num, value="found_by"
            )
            cell.font = font_bold
            col_num += 1
            ...
            col_num += 1
            cell = worksheet.cell(
                row=row_num, column=col_num, value="engagement"
            )
            cell.font = font_bold
            col_num += 1
            cell = worksheet.cell(
                row=row_num, column=col_num, value="product_id"
            )
            cell.font = font_bold
            col_num += 1
            ...
            col_num += 1
            cell = worksheet.cell(
                row=row_num, column=col_num, value="endpoints"
            )
            cell.font = font_bold
            col_num += 1
            cell = worksheet.cell(
        ...
        if row_num > 1:
            ...
            worksheet.cell(
                row=row_num,
                column=col_num,
                value=finding.test.engagement.product.id,
            )
            col_num += 1
            worksheet.cell(
                row=row_num,
            ...
            worksheet.cell(
                row=row_num, column=col_num, value=vulnerability_ids_value
            )
