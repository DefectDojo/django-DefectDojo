...
class CoverPageForm(forms.Form):
    heading = forms.CharField(max_length=200, required=True, help_text="The main report heading.")
...
class TableOfContentsForm(forms.Form):
    heading = forms.CharField(max_length=200, required=True, initial="Table of Contents")
    depth = forms.IntegerField(min_value=1, max_value=6, initial=4)
...
class ExecutiveSummaryForm(forms.Form):
    heading = forms.CharField(max_length=200, required=True, initial="Executive Summary")
    content = forms.CharField(widget=Div(attrs={'class': 'editor'}))
...
def report_widget_factory(json_data=None, request=None, user=None):
    ...
    for idx, widget in enumerate(widgets):
        ...
        if widget.keys()[0] == 'executive-summary':
            ...
            selected_widgets[widget.keys()[0]] = exec_summary
