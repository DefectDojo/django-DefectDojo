...
class TableOfContentsForm(forms.Form):
    ...
    depth = forms.IntegerField(
        min_value=1, required=False, max_value=6, initial=4
    )
    ...
    def render(self, name, value, attrs=None, renderer=None):
        ...
        return format_html(
            ...
            '<a class="btn btn-default" data-edit="italic" \
                title="Italic (Ctrl/Cmd+I)"><i class="fa-solid fa-italic"></i></a>'
            ...
            '<a class="btn btn-default" data-edit="outdent" \
                title="Reduce indent (Shift+Tab)"><i class="fa-solid fa-outdent">'
            '</i></a><a class="btn btn-default" data-edit="indent" \
                title="Indent (Tab)"><i class="fa-solid fa-indent"></i>'
            ...
            '<i class="fa-solid fa-rotate-left"></i></a><a class="btn btn-default" \
                data-edit="redo" title="Redo (Ctrl/Cmd+Y)">'
...
class WYSIWYGContentForm(forms.Form):
    heading = forms.CharField(
        max_length=200, required=False, initial="WYSIWYG Content"
    )
    content = forms.CharField(
        required=False, widget=Div(attrs={"class": "editor"})
    )
...
class CoverPage(Widget):
    def __init__(self, *args, **kwargs):
        ...
        self.help_text = (
            "The cover page includes a page break after its content."
        )
...
class TableOfContents(Widget):
    ...
    def get_html(self):
        return render_to_string(
            ...
            {"title": self.title, "depth": self.depth},
        )
...
class FindingList(Widget):
    def __init__(self, *args, **kwargs):
        ...
        if self.request is not None:
            self.paged_findings = get_page_items(
                self.request, self.findings.qs, 25
            )
        else:
...
class EndpointList(Widget):
    def __init__(self, *args, **kwargs):
        ...
        self.multiple = "false"
        if self.request is not None:
            self.paged_endpoints = get_page_items(
                self.request, self.endpoints.qs, 25
            )
        else:
    ...
    for idx, widget in enumerate(widgets):
        ...
        if list(widget.keys())[0] == "endpoint-list":
            ...
            endpoints = EndpointFilter(
                d, queryset=endpoints, user=request.user
            )
