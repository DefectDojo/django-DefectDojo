...
class DojoGroupUserSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        if self.instance is not None and \
                ...
                not user_has_permission(self.context['request'].user, data.get('group'), Permissions.Group_Manage_Users):
        ...
        if self.instance is None or \
            ...
            members = Dojo_Group_User.objects.filter(group=data.get('group'), user=data.get('user'))
            if members.count() > 0:
                raise ValidationError('Dojo_Group_User already exists')

        if self.instance is not None and not data.get('role').is_owner:
            owners = Dojo_Group_User.objects.filter(group=data.get('group'), role__is_owner=True).exclude(id=self.instance.id).count()
            if owners < 1:
