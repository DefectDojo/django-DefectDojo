...
* **Manage > Rules Engine > Rules (Classic)** is the original Rules Engine, superseded by Rules Engine 2.0. It carries the badge only on instances that have both engines turned on, since there is nothing to move to otherwise. See the [conversion guide](/automation/rules_engine_2/converting_from_rules_engine/).
