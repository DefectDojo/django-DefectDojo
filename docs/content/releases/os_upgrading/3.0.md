...
> **DefectDojo Pro:** you can also enable Locations from the **Settings > Feature Flags** page without setting the environment variable — the toggle drives the Pro UI and the import pipeline live, with no restart. That toggle is **enable-only**: it turns Locations on but cannot turn it back off. The `DD_V3_FEATURE_LOCATIONS` deployment setting still governs the Classic UI and the `/api/v2` endpoint/location route wiring, which are fixed when the process starts, so keep it in sync if you rely on those surfaces. See [Feature Flags](/admin/feature_flags/pro__feature_flags/).
...
Set `DD_V3_FEATURE_LOCATIONS=False` to return to the legacy Endpoint model, UI, and read/write API. Because the original Endpoint rows are never deleted, your pre-upgrade endpoint data is still there. On DefectDojo Pro the **Settings > Feature Flags** toggle is enable-only, so rolling back is done through this deployment setting, not the UI.
