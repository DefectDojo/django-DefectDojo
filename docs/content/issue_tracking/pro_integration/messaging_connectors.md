...
description: "Send alerts from DefectDojo to Slack, Microsoft Teams, or email."
weight: 4
audience: pro
---
...
Messaging Connectors send alerts from DefectDojo to a chat service or to an email address. They sit beside the ticketing and incident-management connectors on the same **Downstream Connectors** page, and they are configured the same way: create a connection once, then decide what should be sent to it.
...
| Vendor | What you provide | How many channels per connection |
| --- | --- | --- |
...
Use it to confirm the things that are easy to get wrong: for Slack, that the channel ID is right and the bot can post there; for Teams, that the workflow URL still works; for email, that the address is deliverable. The vendor's own answer is passed through, so a missing Slack invite reads as a message telling you to invite the bot rather than a generic failure.
...
2. Select **New Alert** and fill in:
   ...
   - **Where it delivers**: the vendor's own destination field, so a Slack channel ID, an optional Teams channel label, or a list of email addresses.
...
4. Add a **Send a Slack Message**, **Send a Microsoft Teams Message**, or **Send an Email** node and set:
   - **Connection**: the messaging connection you created.
   - **Destination**: the vendor's destination, so a channel ID for Slack, an optional channel label for Teams, or recipients for email.
...
A revoked bot token or a deleted workflow fails every alert it serves. Rather than recording the same failure for every event, DefectDojo counts consecutive credential failures per destination and stops sending after a few of them. The connection reports which destination was disabled and why.

To recover: fix the credential (reinstall the Slack app and paste the new token, or recreate the Teams workflow and paste the new URL), then either send a test message to that destination, which re-enables it on success, or use the re-enable action directly.

Only credential failures cause this. A message rejected because a Slack channel ID is wrong, the bot is not invited, or an email address does not exist does not disable anything, because the credential is fine and correcting the destination should work immediately.
