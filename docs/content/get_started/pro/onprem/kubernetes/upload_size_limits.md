...
description: "Why a large scan file fails to upload on Kubernetes, and which limit to raise in the Helm chart"
draft: false
weight: 5
audience: pro
aliases:
  - /get_started/pro/onprem/upload_size_limits/
---

A large scan file can be rejected by more than one limit, at different points in the request path, and the error you get tells you which one you hit. This page covers where those limits are and how to raise them on a Kubernetes deployment. Running Docker Compose? See [Upload Size Limits (Docker Compose)](/get_started/pro/onprem/docker_compose/upload_size_limits/).
