...
Both methods also use Deduplication differently: while two discrete Imported Tests in the same Asset will identify and label duplicate Findings separately, Reimport will not create any Findings it identifies as [duplicates](/triage_findings/finding_deduplication/avoid_excess_duplicates/) within the Test.
