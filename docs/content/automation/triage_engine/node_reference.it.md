...
- /it/automation/rules_engine_2/node_reference/
---
...
Triage Engine include 25 nodi suddivisi in quattro categorie. Questa pagina li documenta tutti.
...
Un nodo di uscita dipende da qualcosa esterno alla regola: un token Slack, un webhook Microsoft Teams, una configurazione JIRA, un connettore con licenza. Quando questo manca o è disattivato, il nodo non può funzionare, e Triage Engine lo segnala in tre momenti diversi invece di fallire silenziosamente:
...
| Impostazione | Predefinito | Note |
|---------|---------|-------|
| **Titolo** | `Triage Engine: {{ctx.rule_name}}` | Renderizzato una volta per l'intero batch. |
