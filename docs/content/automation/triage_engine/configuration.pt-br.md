...
- /pt-br/automation/rules_engine_2/configuration/
---
...
O Triage Engine é configurado separadamente do Rules Engine original. Os dois mecanismos não compartilham nenhum ajuste, portanto uma configuração `DD_RULES_ENGINE_*` não afeta o Triage Engine, e uma configuração `DD_RULES_V2_*` não afeta o mecanismo original.
...
Alguns nós do Triage Engine usam a configuração de integração de todo o sistema, em vez da própria:
