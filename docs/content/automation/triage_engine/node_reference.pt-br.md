...
- /pt-br/automation/rules_engine_2/node_reference/
---

<span style="background-color:rgba(242, 86, 29, 0.3)">Nota: o Triage Engine é um recurso exclusivo do DefectDojo Pro.</span>

O Triage Engine vem com 25 nós em quatro categorias. Esta página documenta todos eles.
...
Um nó de saída depende de algo externo à regra: um token do Slack, um webhook do Microsoft Teams, uma configuração do JIRA, um conector licenciado. Quando isso está ausente ou desligado, o nó não consegue funcionar, e o Triage Engine avisa disso em três momentos diferentes, em vez de falhar silenciosamente:
...
| Setting | Default | Notes |
|---------|---------|-------|
| **Title** | `Triage Engine: {{ctx.rule_name}}` | Renderizado uma vez para o lote inteiro. |
