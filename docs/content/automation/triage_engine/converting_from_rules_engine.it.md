...
- /it/automation/rules_engine_2/converting_from_rules_engine/
---
...
I due motori funzionano fianco a fianco. Attivare Triage Engine non cambia nulla delle regole [Rules Engine](/automation/rules_engine/about/) esistenti, e non c'è una scadenza entro cui spostarle.

Quando si vuole spostarle, esiste un convertitore. Traduce una regola di Rules Engine (un filtro più un elenco ordinato di azioni) in un grafo Triage Engine equivalente.
...
* **Il modello di concorrenza è diverso.** Rules Engine ha un unico lock di esecuzione a livello di istanza. Triage Engine serializza per regola, quindi regole distinte vengono eseguite in concorrenza. Un insieme di regole che prima si alternava ora si sovrapporrà.
* **Un'azione non ha equivalente.** Un'azione "imposta falso positivo a falso" non può essere espressa come nodo di Triage Engine e deve essere convertita a mano.
...
1. Attivare Triage Engine e lasciare in esecuzione le regole esistenti.
