...
- /fr/automation/rules_engine_2/configuration/
---
...
Triage Engine se configure séparément du Rules Engine d'origine. Les deux moteurs ne partagent aucun réglage : un paramètre `DD_RULES_ENGINE_*` n'affecte pas Triage Engine, et un paramètre `DD_RULES_V2_*` n'affecte pas le moteur d'origine.
...
Certains nœuds de Triage Engine utilisent une configuration d'intégration à l'échelle du système plutôt que la leur propre :
