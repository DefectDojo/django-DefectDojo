...
- /zh-hans/automation/rules_engine_2/configuration/
---
...
Triage Engine 与原有的 Rules Engine 分开配置。两套引擎不共享任何调优参数，因此 `DD_RULES_ENGINE_*` 设置不会影响 Triage Engine，`DD_RULES_V2_*` 设置也不会影响原有引擎。
...
部分 Triage Engine 节点使用的是系统级的集成配置，而不是节点自身的配置：
