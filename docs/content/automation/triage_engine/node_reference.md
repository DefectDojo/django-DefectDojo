...
| **Maximum Reports per Run** | `25` | Shown for the two per-item scopes. The node stops after this many reports in one run (at most 100) and records a skipped delivery saying how many Findings or Assets were left out. |
...
Every report is a file and a render, so the per-item scopes stop at **Maximum Reports per Run** rather than the higher per-item ceiling the ticket and notification nodes share. A rule that matches ten thousand Findings produces the first 25 reports and one skipped delivery reporting the rest. Narrow the rule with a condition when you need a specific set.
