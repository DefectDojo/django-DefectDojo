...
Un **run** è una singola esecuzione di una regola. Ogni run viene registrato, sia che abbia avuto esito positivo sia che sia fallito, e ogni nodo al suo interno lascia una traccia. **Triage Engine > Runs** li elenca.
...
Una regola che modifica un Finding produce esattamente il tipo di evento su cui un'altra regola può attivarsi. Triage Engine lo consente, quindi catene del tipo `A -> B -> C` funzionano, e le limita in due modi indipendenti:
