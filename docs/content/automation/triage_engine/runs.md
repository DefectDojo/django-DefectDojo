...
A rule that changes a Finding produces exactly the kind of event another rule can trigger on. Triage Engine allows that, so `A -> B -> C` chains work, and bounds it in two independent ways:
...
Every change an Asset rule makes is attributed to the rule, run and node that made it, and each Asset's own rules-engine history is surfaced in the Pro UI as the Asset twin of the Finding provenance timeline. It is served by an internal, session-authenticated endpoint (`/api/vue/rules_engine_v2/assets/{id}/provenance/`) that backs the UI; that path is not part of the token-authenticated `/api/v2/` API and is not intended for automation. Reading it follows Asset visibility: an Asset the caller cannot see answers 404 rather than an empty timeline.
