...
4. Edit the settings (see [Configuration]({{< ref "/getting_started/configuration" >}})) with the following
    ...
    {{< /highlight >}}
5. Further restrict access by configuring a GitHub Organization name:
    {{< highlight python >}}
    DD_SOCIAL_AUTH_GITHUB_ORG_NAME=(str, 'GitHub Organization name'),
    {{< /highlight >}}
    or a GitHub Team ID:
    {{< highlight python >}}
    DD_SOCIAL_AUTH_GITHUB_TEAM_ID=(str, 'GitHub Team ID'),
    {{< /highlight >}}
    Caution! Without an organization name, or team id, any GitHub account will be able to login.
6. Restart DefectDojo, and you should now see a **Login with GitHub**
    button on the login page.
