...
RUN apt-get update \
    && apt-get install -y sudo git expect wget nano libpq-dev postgresql postgresql-contrib libmysqlclient-dev \
...
RUN git clone https://github.com/DefectDojo/sample-scan-files.git

# Start the DB server and run the app
#CMD sudo chown -R mysql:mysql /var/lib/mysql /var/run/mysqld \
