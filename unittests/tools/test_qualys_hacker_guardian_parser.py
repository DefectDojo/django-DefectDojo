...
class TestQualysHackerGuardianParser(DojoTestCase):
    ...
    def test_qualys_hacker_guardian_parser_with_no_findings(self):
        with (get_unit_tests_scans_path("qualys_hacker_guardian") / "zero_finding.csv").open(encoding="utf-8") as testfile:
            parser = QualysHackerGuardianParser()
            findings = parser.get_findings(testfile, Test())
            self.assertEqual(0, len(findings))

    def test_qualys_hacker_guardian_parser_with_one_findings(self):
        with (get_unit_tests_scans_path("qualys_hacker_guardian") / "one_finding.csv").open(encoding="utf-8") as testfile:
            parser = QualysHackerGuardianParser()
            findings = parser.get_findings(testfile, Test())
            ...
            self.assertEqual(len(self.get_unsaved_locations(finding)), 2)

    def test_qualys_hacker_guardian_parser_with_many_findings(self):
        with (get_unit_tests_scans_path("qualys_hacker_guardian") / "many_finding.csv").open(encoding="utf-8") as testfile:
            parser = QualysHackerGuardianParser()
            findings = parser.get_findings(testfile, Test())
            self.validate_locations(findings)
            ...
            self.assertEqual("Low", finding.severity)
            self.assertIsNotNone(finding.description)
            self.assertEqual(len(self.get_unsaved_locations(finding)), 1)
            finding = findings[2]
