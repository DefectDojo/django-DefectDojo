...
class TestGitleaksParser(DojoTestCase):
    ...
    def test_parse_file_legacy_with_no_findings(self):
        with (get_unit_tests_scans_path("gitleaks") / "no_findings.json").open(encoding="utf-8") as testfile:
            parser = GitleaksParser()
            findings = parser.get_findings(testfile, Test())
            self.assertEqual(0, len(findings))

    def test_parse_file_legacy_with_one_finding(self):
        with (get_unit_tests_scans_path("gitleaks") / "data_one.json").open(encoding="utf-8") as testfile:
            parser = GitleaksParser()
            findings = parser.get_findings(testfile, Test())
    ...
    def test_parse_file_legacy_with_multiple_finding(self):
        with (get_unit_tests_scans_path("gitleaks") / "data_many.json").open(encoding="utf-8") as testfile:
            parser = GitleaksParser()
            findings = parser.get_findings(testfile, Test())
    ...
    def test_parse_file_legacy_with_multiple_redacted_finding(self):
        with (get_unit_tests_scans_path("gitleaks") / "redacted_data_many.json").open(encoding="utf-8") as testfile:
            parser = GitleaksParser()
            findings = parser.get_findings(testfile, Test())
            self.assertEqual(6, len(findings))

    def test_parse_file_legacy_from_issue4336(self):
        with (get_unit_tests_scans_path("gitleaks") / "issue4336.json").open(encoding="utf-8") as testfile:
            parser = GitleaksParser()
            findings = parser.get_findings(testfile, Test())
            self.assertEqual(1, len(findings))
            with self.subTest(i=0):
    ...
    def test_parse_file_from_version_7_5_0(self):
        with (get_unit_tests_scans_path("gitleaks") / "version_7.5.0.json").open(encoding="utf-8") as testfile:
            parser = GitleaksParser()
            findings = parser.get_findings(testfile, Test())
    ...
    def test_parse_file_from_version_8(self):
        with (get_unit_tests_scans_path("gitleaks") / "gitleaks8_many.json").open(encoding="utf-8") as testfile:
            parser = GitleaksParser()
            findings = parser.get_findings(testfile, Test())
