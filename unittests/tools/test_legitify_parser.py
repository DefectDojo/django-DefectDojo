...
class TestLegitifyParser(DojoTestCase):
    def test_parse_file_with_many_findings(self):
        with open(get_unit_tests_path() + "/scans/legitify/legitify_many_findings.json") as testfile:
    ...
    def test_parse_file_with_one_finding(self):
        with open(get_unit_tests_path() + "/scans/legitify/legitify_one_finding.json") as testfile:
            parser = LegitifyParser()
            findings = parser.get_findings(testfile, Test())
    ...
    def test_parse_file_with_no_findings(self):
        with open(get_unit_tests_path() + "/scans/legitify/legitify_no_findings.json") as testfile:
            parser = LegitifyParser()
            findings = parser.get_findings(testfile, Test())
