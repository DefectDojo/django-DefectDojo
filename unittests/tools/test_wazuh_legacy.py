...
from dojo.tools.wazuh_legacy.parser import WazuhLegacyParser
from unittests.dojo_test_case import DojoTestCase, get_unit_tests_scans_path


class TestWazuhLegacyParser(DojoTestCase):

    def test_parse_v4_7_no_findings(self):
        with (get_unit_tests_scans_path("wazuh_legacy") / "v4-7_no_findings.json").open(encoding="utf-8") as testfile:
            parser = WazuhLegacyParser()
            findings = parser.get_findings(testfile, Test())
            self.assertEqual(0, len(findings))

    def test_parse_v4_7_one_finding(self):
        with (get_unit_tests_scans_path("wazuh_legacy") / "v4-7_one_finding.json").open(encoding="utf-8") as testfile:
            parser = WazuhLegacyParser()
            findings = parser.get_findings(testfile, Test())
    ...
    def test_parse_v4_7_many_finding(self):
        with (get_unit_tests_scans_path("wazuh_legacy") / "v4-7_many_findings.json").open(encoding="utf-8") as testfile:
            parser = WazuhLegacyParser()
            findings = parser.get_findings(testfile, Test())
            for finding in findings:
                for endpoint in finding.unsaved_endpoints:
    ...
    def test_parse_v4_7_one_finding_with_endpoint(self):
        with (get_unit_tests_scans_path("wazuh_legacy") / "v4-7_one_finding_with_endpoint.json").open(encoding="utf-8") as testfile:
            parser = WazuhLegacyParser()
            findings = parser.get_findings(testfile, Test())
            for finding in findings:
                for endpoint in finding.unsaved_endpoints:
