...
class TestAcunetixParser(DojoTestCase):
    ...
    def test_parse_file_with_one_finding_false_positive(self):
        testfile = open("unittests/scans/acunetix/acunetix360_one_finding_false_positive.json")
        parser = AcunetixParser()
        findings = parser.get_findings(testfile, Test())
        self.assertEqual(1, len(findings))
        for finding in findings:
    ...
    def test_parse_file_with_one_finding_risk_accepted(self):
        testfile = open("unittests/scans/acunetix/acunetix360_one_finding_accepted_risk.json")
        parser = AcunetixParser()
        findings = parser.get_findings(testfile, Test())
        self.assertEqual(1, len(findings))
        for finding in findings:
    ...
    def test_parse_file_with_mulitple_cwe(self):
        testfile = open("unittests/scans/acunetix/acunetix360_multiple_cwe.json")
        parser = AcunetixParser()
        findings = parser.get_findings(testfile, Test())
        self.assertEqual(1, len(findings))
        for finding in findings:
