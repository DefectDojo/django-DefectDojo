...
class TestApiVulnersParser(DojoTestCase):
    ...
    def test_parse_many_findings(self):
        with (get_unit_tests_scans_path("api_vulners") / "report_many_vulns.json").open(encoding="utf-8") as testfile:
            ...
            self.assertEqual("223.234.234.123", self.get_unsaved_locations(finding)[0].host)
    ...
    def test_parse_one_finding(self):
        with (get_unit_tests_scans_path("api_vulners") / "report_one_vuln.json").open(encoding="utf-8") as testfile:
            parser = ApiVulnersParser()
            findings = parser.get_findings(testfile, Test())
            self.validate_locations(findings)
            ...
            self.assertEqual("12.34.56.78", self.get_unsaved_locations(finding)[0].host)
    ...
    def test_parse_no_finding(self):
        with (get_unit_tests_scans_path("api_vulners") / "report_no_vulns.json").open(encoding="utf-8") as testfile:
            parser = ApiVulnersParser()
            findings = parser.get_findings(testfile, Test())
            self.assertEqual(0, len(findings))

    def test_parse_no_description(self):
        with (get_unit_tests_scans_path("api_vulners") / "report_no_description.json").open(encoding="utf-8") as testfile:
            parser = ApiVulnersParser()
            findings = parser.get_findings(testfile, Test())
            self.validate_locations(findings)
            self.assertEqual(1, len(findings))
