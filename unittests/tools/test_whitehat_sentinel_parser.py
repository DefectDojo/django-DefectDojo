...
class TestWhiteHatSentinelParser(DojoTestCase):
    ...
    def test_parse_file_with_no_vuln_has_no_findings(self):
        with self.assertRaises(ValueError), \
          (get_unit_tests_scans_path("whitehat_sentinel") / "empty_file.json").open(encoding="utf-8") as testfile:
            parser = WhiteHatSentinelParser()
            parser.get_findings(testfile, Test())

    def test_parse_file_with_one_vuln_has_one_findings(self):
        with (get_unit_tests_scans_path("whitehat_sentinel") / "one_vuln.json").open(encoding="utf-8") as testfile:
            parser = WhiteHatSentinelParser()
    ...
    def test_parse_file_with_invalid_data(self):
        with self.assertRaises(ValueError), \
          (get_unit_tests_scans_path("whitehat_sentinel") / "invalid_data.txt").open(encoding="utf-8") as testfile:
            parser = WhiteHatSentinelParser()
            parser.get_findings(testfile, Test())
