...
class TestCyclonedxParser(DojoTestCase):
    def test_grype_report(self):
        with open(get_unit_tests_scans_path("cyclonedx") / "grype_dd_1_14_1.xml", encoding="utf-8") as file:
    ...
    def test_spec1_report(self):
        ...
        with open(get_unit_tests_scans_path("cyclonedx") / "spec1.xml", encoding="utf-8") as file:
            parser = CycloneDXParser()
            findings = list(parser.get_findings(file, Test()))
            for finding in findings:
                self.assertIn(finding.severity, Finding.SEVERITIES)
    ...
    def test_spec1_report_low_first(self):
        """Test a report from the spec itself"""
        with open(get_unit_tests_scans_path("cyclonedx") / "spec1_lowfirst.xml", encoding="utf-8") as file:
            parser = CycloneDXParser()
            findings = list(parser.get_findings(file, Test()))
            for finding in findings:
                self.assertIn(finding.severity, Finding.SEVERITIES)
    ...
    def test_cyclonedx_bom_report(self):
        with open(get_unit_tests_scans_path("cyclonedx") / "cyclonedx_bom.xml", encoding="utf-8") as file:
            parser = CycloneDXParser()
    ...
    def test_cyclonedx_jake_report(self):
        ...
        with open(get_unit_tests_scans_path("cyclonedx") / "jake.xml", encoding="utf-8") as file:
            parser = CycloneDXParser()
            findings = parser.get_findings(file, Test())
            for finding in findings:
                self.assertIn(finding.severity, Finding.SEVERITIES)
    ...
    def test_cyclonedx_retirejs_report(self):
        ...
        with open(get_unit_tests_scans_path("cyclonedx") / "retirejs.latest.xml", encoding="utf-8") as file:
            parser = CycloneDXParser()
            findings = parser.get_findings(file, Test())
            for finding in findings:
                self.assertIn(finding.severity, Finding.SEVERITIES)
    ...
    def test_cyclonedx_grype_11_report(self):
        ...
        with open(get_unit_tests_scans_path("cyclonedx") / "dd_1_15_0.xml", encoding="utf-8") as file:
            parser = CycloneDXParser()
            findings = parser.get_findings(file, Test())
            for finding in findings:
                self.assertIn(finding.severity, Finding.SEVERITIES)
    ...
    def test_cyclonedx_1_4_xml(self):
        ...
        with open(get_unit_tests_scans_path("cyclonedx") / "valid-vulnerability-1.4.xml", encoding="utf-8") as file:
            parser = CycloneDXParser()
            findings = parser.get_findings(file, Test())
            for finding in findings:
                self.assertIn(finding.severity, Finding.SEVERITIES)
    ...
    def test_cyclonedx_1_4_json(self):
        ...
        with open(get_unit_tests_scans_path("cyclonedx") / "valid-vulnerability-1.4.json", encoding="utf-8") as file:
            parser = CycloneDXParser()
            findings = parser.get_findings(file, Test())
            for finding in findings:
                self.assertIn(finding.severity, Finding.SEVERITIES)
    ...
    def test_cyclonedx_1_4_jake_json(self):
        ...
        with open(get_unit_tests_scans_path("cyclonedx") / "jake2.json", encoding="utf-8") as file:
            parser = CycloneDXParser()
            findings = parser.get_findings(file, Test())
    ...
    def test_cyclonedx_1_4_xml_cvssv31(self):
        """CycloneDX version 1.4 XML format"""
        with open(get_unit_tests_scans_path("cyclonedx") / "log4j.xml", encoding="utf-8") as file:
            parser = CycloneDXParser()
            findings = parser.get_findings(file, Test())
            for finding in findings:
                self.assertIn(finding.severity, Finding.SEVERITIES)
    ...
    def test_cyclonedx_1_4_json_cvssv31(self):
        """CycloneDX version 1.4 JSON format"""
        with open(get_unit_tests_scans_path("cyclonedx") / "log4j.json", encoding="utf-8") as file:
            parser = CycloneDXParser()
            findings = parser.get_findings(file, Test())
            for finding in findings:
                self.assertIn(finding.severity, Finding.SEVERITIES)
    ...
    def test_cyclonedx_1_4_json_nested_cvssv31(self):
        """CycloneDX version 1.4 JSON format"""
        with open(get_unit_tests_scans_path("cyclonedx") / "nested-component-log4j.json", encoding="utf-8") as file:
            parser = CycloneDXParser()
            findings = parser.get_findings(file, Test())
            for finding in findings:
                self.assertIn(finding.severity, Finding.SEVERITIES)
    ...
    def test_cyclonedx_issue_9277(self):
        ...
        with open(get_unit_tests_scans_path("cyclonedx") / "issue_9277.json", encoding="utf-8") as file:
            parser = CycloneDXParser()
            findings = parser.get_findings(file, Test())
            for finding in findings:
                self.assertIn(finding.severity, Finding.SEVERITIES)
    ...
    def test_cyclonedx_issue_8022(self):
        """CycloneDX version 1.4 JSON format"""
        with open(get_unit_tests_scans_path("cyclonedx") / "issue_8022.json", encoding="utf-8") as file:
            parser = CycloneDXParser()
            findings = parser.get_findings(file, Test())
            for finding in findings:
                self.assertIn(finding.severity, Finding.SEVERITIES)
