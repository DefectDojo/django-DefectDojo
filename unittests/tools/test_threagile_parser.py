...
class TestThreAgileParser(DojoTestCase):
    def test_non_threagile_file_raises_error(self):
        with (get_unit_tests_scans_path("threagile") / "bad_formatted_risks_file.json").open(encoding="utf-8") as testfile:
    ...
    def test_empty_file_returns_no_findings(self):
        with (get_unit_tests_scans_path("threagile") / "empty_file_no_risks.json").open(encoding="utf-8") as testfile:
            parser = ThreagileParser()
            findings = parser.get_findings(testfile, Test())
            self.assertEqual(0, len(findings))

    def test_file_with_vulnerabilities_returns_correct_findings(self):
        with (get_unit_tests_scans_path("threagile") / "risks.json").open(encoding="utf-8") as testfile:
            parser = ThreagileParser()
            findings = parser.get_findings(testfile, Test())
