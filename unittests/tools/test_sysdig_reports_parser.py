...
class TestSysdigParser(DojoTestCase):
    ...
    def test_sysdig_parser_with_one_criticle_vuln_has_one_findings_legacy(self):
        with open(get_unit_tests_scans_path("sysdig_reports") / "legacy_scanner" / "sysdig_reports_one_vul.csv", encoding="utf-8") as testfile:
            parser = SysdigReportsParser()
            findings = parser.get_findings(testfile, Test())
            for finding in findings:
                for endpoint in finding.unsaved_endpoints:
    ...
    def test_sysdig_parser_missing_cve_field_id_from_csv_file_legacy(self):
        with self.assertRaises(ValueError) as context, \
          open(get_unit_tests_scans_path("sysdig_reports") / "legacy_scanner" / "sysdig_reports_missing_cve_field.csv", encoding="utf-8") as testfile:
            parser = SysdigReportsParser()
            findings = parser.get_findings(testfile, Test())
            for finding in findings:
                for endpoint in finding.unsaved_endpoints:
    ...
    def test_sysdig_parser_missing_cve_field_not_starting_with_cve_legacy(self):
        with self.assertRaises(ValueError) as context, \
          open(get_unit_tests_scans_path("sysdig_reports") / "legacy_scanner" / "sysdig_reports_not_starting_with_cve.csv", encoding="utf-8") as testfile:
            parser = SysdigReportsParser()
            findings = parser.get_findings(testfile, Test())
            for finding in findings:
                for endpoint in finding.unsaved_endpoints:
    ...
    def test_sysdig_parser_json_with_many_findings_legacy(self):
        with open(get_unit_tests_scans_path("sysdig_reports") / "legacy_scanner" / "sysdig.json", encoding="utf-8") as testfile:
            parser = SysdigReportsParser()
            findings = parser.get_findings(testfile, Test())
            for finding in findings:
                for endpoint in finding.unsaved_endpoints:
