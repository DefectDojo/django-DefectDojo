...
class TestInvictiParser(DojoTestCase):
    ...
    def test_parse_file_with_one_finding(self):
        with (get_unit_tests_scans_path("invicti") / "invicti_one_finding.json").open(encoding="utf-8") as testfile:
    ...
    def test_parse_file_with_multiple_finding(self):
        with (get_unit_tests_scans_path("invicti") / "invicti_many_findings.json").open(encoding="utf-8") as testfile:
            parser = InvictiParser()
            findings = parser.get_findings(testfile, Test())
    ...
    def test_parse_file_issue_9816(self):
        with (get_unit_tests_scans_path("invicti") / "issue_9816.json").open(encoding="utf-8") as testfile:
            parser = InvictiParser()
            findings = parser.get_findings(testfile, Test())
    ...
    def test_parse_file_issue_10311(self):
        with (get_unit_tests_scans_path("invicti") / "issue_10311.json").open(encoding="utf-8") as testfile:
            parser = InvictiParser()
            findings = parser.get_findings(testfile, Test())
            self.assertEqual(3, len(findings))
            for finding in findings:
