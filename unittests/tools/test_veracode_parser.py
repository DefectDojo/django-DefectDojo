...
class TestVeracodeScannerParser(DojoTestCase):
    ...
    def parse_csv(self):
        testfile = open("unittests/scans/veracode/veracode_sca.csv")
        parser = VeracodeParser()
        findings = parser.get_findings(testfile, Test())
        self.assertEqual(3, len(findings))
    ...
    def parse_json(self):
        testfile = open("unittests/scans/veracode/veracode_sca.json")
        parser = VeracodeParser()
        findings = parser.get_findings(testfile, Test())
        self.assertEqual(1, len(findings))
    ...
    def parse_json_fixed(self):
        testfile = open("unittests/scans/veracode/veracode_sca_fixed.json")
        parser = VeracodeParser()
        findings = parser.get_findings(testfile, Test())
        self.assertEqual(1, len(findings))
