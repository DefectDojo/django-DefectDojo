...
class TestNexposeParser(DojoTestCase):
    ...
    def test_nexpose_parser_has_many_finding(self):
        ...
        with (get_unit_tests_scans_path("nexpose") / "many_vulns.xml").open(encoding="utf-8") as testfile:
            ...
            # vuln 5 - location
            location = self.get_unsaved_locations(finding)[0]
            self.assertEqual(22, location.port)
            self.assertEqual("ssh", location.protocol)
            ...
            self.assertEqual("udp", self.get_unsaved_locations(finding)[0].protocol)
            ...
            # vuln 37 - location
            location = self.get_unsaved_locations(finding)[0]
            self.assertEqual(137, location.port)
            self.assertEqual("udp", location.protocol)

    def test_nexpose_parser_tests_outside_location(self):
        with (get_unit_tests_scans_path("nexpose") / "report_auth.xml").open(encoding="utf-8") as testfile:
            ...
            self.assertEqual(16, len(self.get_unsaved_locations(finding)))
            self.assertIsNone(finding.unsaved_vulnerability_ids)
    ...
    def test_nexpose_parser_dns(self):
        with (get_unit_tests_scans_path("nexpose") / "dns.xml").open(encoding="utf-8") as testfile:
            ...
            self.assertEqual("dns", str(self.get_unsaved_locations(finding)[0].protocol))
            self.assertEqual("tcp", str(self.get_unsaved_locations(finding)[0].fragment))
            self.assertEqual("dns", str(self.get_unsaved_locations(finding)[1].protocol))
            self.assertEqual("udp", str(self.get_unsaved_locations(finding)[1].fragment))
            self.assertEqual("dns://192.168.1.1#tcp", str(self.get_unsaved_locations(finding)[0]))
            self.assertEqual("dns://192.168.1.1#udp", str(self.get_unsaved_locations(finding)[1]))
            ...
            self.assertEqual(1, len(self.get_unsaved_locations(finding)))
            self.assertEqual("dns", str(self.get_unsaved_locations(finding)[0].protocol))
            self.assertEqual("udp", str(self.get_unsaved_locations(finding)[0].fragment))
            self.assertEqual("dns://192.168.1.1#udp", str(self.get_unsaved_locations(finding)[0]))
