...
class TestDetectSecretsParser(DojoTestCase):
    ...
    def test_parse_no_findings(self):
        with (get_unit_tests_scans_path("detect_secrets") / "no_findings.json").open(encoding="utf-8") as testfile:
            parser = DetectSecretsParser()
            findings = parser.get_findings(testfile, Test())
            self.assertEqual(0, len(findings))

    def test_parse_many_findings(self):
        with (get_unit_tests_scans_path("detect_secrets") / "many_findings.json").open(encoding="utf-8") as testfile:
            parser = DetectSecretsParser()
            findings = parser.get_findings(testfile, Test())
