...
from unittests.dojo_test_case import DojoAPITestCase, get_unit_tests_path, skip_unless_v2, skip_unless_v3
from unittests.test_utils import assertImportModelsCreated
...
class EndpointMetaImportMixin:
    ...
    def test_endpoint_meta_import_endpoint_create_tag_create_meta_create(self):
        location_count_before = self.db_location_count()
        location_tag_count_before = self.db_location_tag_count()
        ...
        self.assertEqual(location_count_before + 3, self.db_location_count())
        self.assertEqual(location_tag_count_before + 6, self.db_location_tag_count())
    ...
    def test_endpoint_meta_import_tag_remove_column(self):
        ...
        self.assertEqual(location_count_before, self.db_location_count())
        self.assertEqual(location_tag_count_before, self.db_location_tag_count())
    ...
    def test_endpoint_meta_import_tag_added_column(self):
        ...
        self.assertEqual(location_tag_count_before + 3, self.db_location_tag_count())
    ...
    def test_endpoint_meta_import_tag_changed_column(self):
        ...
        location_count_before = self.db_location_count()
        location_tag_count_before = self.db_location_tag_count()
        endpoints = self.get_product_endpoints_api(1, host=self.updated_tag_host)["results"]
        self.assertEqual(len(endpoints), 1, endpoints)
...
@skip_unless_v2
class EndpointMetaImportMixinWithEndpoints:
    def __init__(self, *args, **kwargs):
        self.meta_import_full = get_unit_tests_path() / "endpoint_meta_import" / "full_endpoint_meta_import.csv"
        self.meta_import_no_hostname = get_unit_tests_path() / "endpoint_meta_import" / "no_hostname_endpoint_meta_import.csv"
        self.meta_import_updated_added = get_unit_tests_path() / "endpoint_meta_import" / "updated_added_endpoint_meta_import.csv"
...
# TODO: Delete this after the move to Locations
class EndpointMetaImportTestAPIWithEndpoints(DojoAPITestCase, EndpointMetaImportMixinWithEndpoints):
    fixtures = ["dojo_testdata.json"]

    def __init__(self, *args, **kwargs):
        # TODO: remove __init__ if it does nothing...
        EndpointMetaImportMixinWithEndpoints.__init__(self, *args, **kwargs)
        # super(EndpointMetaImportMixin, self).__init__(*args, **kwargs)
        # super(DojoAPITestCase, self).__init__(*args, **kwargs)
        super().__init__(*args, **kwargs)
...
# TODO: Delete this after the move to Locations
class EndpointMetaImportTestUIWithEndpoints(DojoAPITestCase, EndpointMetaImportMixinWithEndpoints):
    fixtures = ["dojo_testdata.json"]
    client_ui = Client()
