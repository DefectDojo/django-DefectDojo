...
current user has `block_execution=True` or the caller passes `sync=True`.
"""
...
class TestForceAsync(DojoTestCase):
    ...
    def test_force_async_true_overrides_sync(self):
        """force_async=True wins even when sync=True is also present."""
        self.assertTrue(we_want_async(sync=True, force_async=True))
    ...
    def test_sync_still_honoured_without_force_async(self):
        """Existing sync=True behavior is unchanged."""
        self.assertFalse(we_want_async(sync=True))
