...
class FlexibleImportTestAPI(DojoAPITestCase):
    ...
    def test_import_with_invalid_parameters(self):
        ...
        with self.subTest('invalid engagement, but exists in another product'):
            ...
            self.assertEqual(import0, ["Engagement 'Engagement 1' does not exist in Product '%s'" % another_product_name])
        ...
        with self.subTest('autocreate product but no product type name'):
            ...
            self.assertEqual(import0, ["Product '%s' does not exist and no product_type_name provided to create the new product in" % PRODUCT_NAME_NEW])
