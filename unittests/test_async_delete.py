...
class TestAsyncDelete(DojoTestCase):
    ...
    These tests use deduplication_execution_mode="sync" and crum.impersonate to run tasks synchronously,
    which allows errors to surface immediately rather than being lost in background workers.
    """

    def setUp(self):
        """Set up test user with deduplication_execution_mode="sync" and disable unneeded features."""
        super().setUp()

        # Create test user with deduplication_execution_mode="sync" to run tasks synchronously
