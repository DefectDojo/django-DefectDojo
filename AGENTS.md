...
All work targets `dev`. Every release, the weekly patch (`X.Y.100`, `X.Y.200`, ...) and
the monthly minor (`X.Y.0`), is cut from `dev` and merged into `master`. There is no
separate patch branch and no hotfix path off `master`.

| Branch | Ships in | Base work here when |
|--------|----------|---------------------|
| `dev` | the next release, patch or minor, whichever the schedule puts first | always: bug fixes, regressions, features, refactors, schema/model changes |
| `master` | already released | never, except an explicit release task |

```bash
git branch --show-current
# Is a topic branch on the working line? Ancestry, not the branch name:
git merge-base --is-ancestor origin/dev HEAD 2>/dev/null && echo "contains origin/dev"
```

Rules:

- **Base every change on `dev`.** Bug fixes and features both go there, and ship in the
  next release from `dev`.
- Judge a topic branch by whether it *contains* `origin/dev`, not by its name. A branch
  cut from `master` or an old release branch is not on the working line: say so and offer
...
for the released line (a release task), and wait for the answer. On confirmation, run
the `touch` command the hook prints; that command is deliberately not allowlisted, so
approving its prompt *is* the confirmation. Without confirmation, move
the work to `dev` first: `git switch -c <name> origin/dev`.
...
Set the milestone in the same step that opens the PR, not in a later cleanup pass. Every
PR targets `dev`, and the next release from `dev` is whichever release the schedule puts
first, patch (`X.Y.<hundreds>`) or minor (`X.Y.0`). So the milestone is the earliest
**open** milestone of either kind whose due date is still in the future. A milestone due
today or overdue is usually a release already cut, so a PR opened now will not make it.
Read the answer out of the repo instead of from memory:

```bash
gh api repos/DefectDojo/django-DefectDojo/milestones --paginate --jq '[.[] | select(.state=="open" and .due_on > (now|todate))] | sort_by(.due_on)[0].title'
```
